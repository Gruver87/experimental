...
impl StoreInner {
    fn new(max_size: usize, min_fee: f64) -> Self {
        Self {
            txs: HashMap::new(),
...
impl MempoolStore {
    #[new]
    #[pyo3(signature = (max_size=10000, min_fee=0.0001))]
    fn new(max_size: usize, min_fee: f64) -> Self {
        Self {
            inner: Mutex::new(StoreInner::new(max_size, min_fee)),
        }
    }
    ...
    #[pyo3(signature = (limit=100, min_fee=0.0))]
    fn get_sorted(&self, py: Python<'_>, limit: usize, min_fee: f64) -> PyResult<PyObject> {
        let guard = self
            .inner
            .lock()
            .map_err(|_| pyo3::exceptions::PyRuntimeError::new_err("mempool_store_lock_poisoned"))?;
...
mod tests {
    ...
    fn entry(hash: &str, fee: f64) -> TxEntry {
        TxEntry {
            tx_hash: hash.into(),
            from_addr: "0xa".into(),
            to_addr: "0xb".into(),
    ...
    fn insert_reject_duplicate_and_low_fee() {
        let mut s = StoreInner::new(10, 0.001);
        assert!(s.insert(entry("h1", 0.01)));
        assert!(!s.insert(entry("h1", 0.02)));
        assert!(!s.insert(entry("h2", 0.0001)));
        assert_eq!(s.size(), 1);
    }

    #[test]
    fn sort_fee_desc() {
        let mut s = StoreInner::new(10, 0.0);
        assert!(s.insert(entry("low", 1.0)));
        assert!(s.insert(entry("high", 9.0)));
        assert!(s.insert(entry("mid", 5.0)));
