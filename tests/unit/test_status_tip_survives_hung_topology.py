#!/usr/bin/env python3
"""GET /status must not block on p2p.get_topology() (48h soak /status timeouts)."""
...
class _HangTopologyP2P:
    ...
    def get_topology(self):
        time.sleep(30)
        return {"topology_healthy": True, "peer_count": 2, "security": {}}
...
def test_status_returns_while_topology_hangs(tmp_path, monkeypatch):
    monkeypatch.setenv("JWT_SECRET", "unit-test-status-topology-hang-secret-32b")
    ...
    RESTHandler.p2p = _HangTopologyP2P()
    ...
    try:
        ...
        with urllib.request.urlopen(
            f"http://127.0.0.1:{cfg.http_port}/status", timeout=2.5
        ) as resp:
            body = json.loads(resp.read().decode())
        elapsed = time.perf_counter() - started
        assert elapsed < 2.0, f"GET /status blocked on topology ({elapsed:.2f}s)"
        assert int(body.get("peers") or 0) == 2
        summary = body.get("p2p_summary") or {}
        assert summary.get("topology_deferred") is True
        sec = summary.get("security") or {}
        assert int(sec.get("rate_limit_per_sec") or 0) == 10
        assert int(sec.get("max_message_bytes") or 0) == 65536
