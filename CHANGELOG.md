...
- **Ops Console UI (2026-09-13):** same-origin `web/console/` (CSP, path-traversal refuse) — live `/metrics` tiles + sparklines, under_mesh/mesh honesty, mempool_store, EIP-1193 Absolute chain switch + `eth_sendTransaction`, council watch (778889), dark/light theme. `/` → console; `/explorer` → legacy. Grafana panels for `abs_p2p_under_mesh` / sync gap. Not soak / not mainnet.
