...
**Hybrid industrial tags:** `v1.3.*` live in [Ultimate Hybrid](https://github.com/Gruver87/Absolute_Blockchain_Ultimate_Hybrid) вЂ” not this Releases page.
...
- **LR STRICT probe harden (2026-09-25):** multi-node lab `HeavyProbe` (prod HTTP timeouts), dual ready+status live recovery, Strict mesh Partial stays WARN, status_slow soft under Strict. Closes lr48pass1 fail_line killers (13x ready TIMEOUT) before LR STRICT 48h. Gate needles + STRICT_SOAK_PARITY. Soak not started.
- **Libp2p STRICT 48h PASS (2026-09-23в†’25):** pack [`docs/evidence/runs/lp2pstrict1/`](docs/evidence/runs/lp2pstrict1/) вЂ” `passed=true` strict=true hard_fails=0 mesh_warn=0 warn_lines=0 fail_lines=0 mesh_ok=2800 tip ~57209в†’~68082 IntervalSec=60 FullHarnessEvery=6. Soft tip_skew/transient_delta OK-only. Harden: peer_probe 3Г—, under_mesh reconnect, soft-only harness never Strict FAIL; scripts `start_soak_prod_mesh_48h_strict.ps1` / `start_soak_evm_mesh_48h_strict.ps1` / LR `-Strict`. Triage: [`docs/sprouts/STRICT_SOAK_PARITY.md`](docs/sprouts/STRICT_SOAK_PARITY.md). **Not** mempool sidecar / not default ind48pass1 / not mainnet / not BLS.
- **Industrial tip 48h PASS + ADR 0021 mesh proof (2026-09-21в†’23):** Experimental prod mesh soak on git `719deb4` / image `23a4a8fa` вЂ” pack [`docs/evidence/runs/ind48pass1/`](docs/evidence/runs/ind48pass1/) (`passed=true`, `hard_fails=0`, `mesh_warn=0`, `warn_lines=0`, tip ~46099в†’~56972, 575 mesh-ok cycles). Proves ADR 0021 wire satoshi + `p2p_mempool_require_wire_satoshi` under live mesh load after industrial HIGH/MED/CI heal. ADR honesty + EVIDENCE_MATRIX / EXECUTION_ORDER / FUND_READINESS updated. **Not** STRICT mempool sidecar (see [`mempool48pass1`](docs/evidence/runs/mempool48pass1/)). **Not** mainnet / not Hybrid / not BLS.
- **CI green restore (2026-09-21):** `rustls` 0.23.45 (`RUSTSEC-2026-0285`); `cargo fmt` abs_native; quote YAML step name in `experimental-rd.yml`. Fund diligence card: [`docs/FUND_READINESS.md`](docs/FUND_READINESS.md). Superseded for mesh claim by `ind48pass1`.
- **verify_hard_all green + registry restore (2026-09-21):** industrial waves needle `native TCP+TLS uses ``_native_listener```; DR `backup_db_drill` handles fail-closed `backup_to` raise; demote tests re-`bootstrap_native_capabilities()` after `reset_for_tests` (stopped process-wide `abs_native missing` pollution в‰€40 P2P/native fails). Honesty/needle drift aligned (PBS off default, stake satoshi, PQ NotImplemented, `/status` `kernels_deferred`, Wave J/K wire helpers in `mempool_wire`). Evidence: `verify_industrial_waves --skip-gate --skip-pytest` PASS (542 needles); `pytest tests/` **2734 passed / 11 skipped**. Mesh: `docker_prod_3node -KeepVolumes` + `probe_prod_mesh -Quick` **RESULT: OK** (tip ~44442 aligned); `verify_pre_soak -SkipPrepare` **PASS** (soak not started). `verify_pre_soak.ps1` ASCII-only RESULT line (PowerShell encoding). **Not** 48h soak.
- **Monitor HOL fix (2026-09-21):** in-node `monitor.py` polls `/status?probe=1` (5s) instead of full `/status` (30s) вЂ” live mesh showed CRITICAL spam + `/health/live` starvation under piled HOL. Debounced CRITICAL (120s). Units: `tests/unit/test_monitor_probe_honesty.py`. **Requires image rebuild** to land on running mesh; restart alone clears stuck threads. **Not** 48h soak.
- **MED wire satoshi + backup fail-closed (2026-09-21):** `p2p_mempool_require_wire_satoshi=True` (default) refuses float-only P2P ingress (`amount_satoshi_required` / `fee_satoshi_required`); dual-write ABS floats remain display-only. Ops `backup_to` raises `PersistError`; `chain_backup` no longer silent-`copy2` after failed SQLite backup. Units: `test_adr0021_wire_satoshi_cutover` + `test_persist_fail_closed` + `test_database_atomic`. **Not** mesh probe / **not** 48h soak.
- **P0 ledger / slash honesty (2026-09-21):** `ConsensusEngine.slash_validator` deactivates proposer/attest selection; adapter ejects engine before persist. ATXV/ATXR **v2** store `value/fee/burned_satoshi` as i64 (v1 dual-read). MempoolStore round-trips `amount_satoshi` and **refuses** missing `fee_satoshi` (no IEEEГ—1e6). Units: `tests/unit/test_p0_slash_satoshi_ledger.py`. **Not** mesh probe / **not** 48h soak.
- **Industrial HIGH honesty (2026-09-21):** slash `mark_slashed` fail-closed (#18); mempool demote refuses under `ABS_NATIVE_MODE=require` before local mutate (#19вЂ“20); bridge reject surfaces `event_bus_emit_failed` (#24); libp2p dial refuses without rust вЂ” stub handle removed (#22). Operator: `.\scripts\verify_industrial_high_honesty.ps1`. **Not** mesh probe / **not** 48h soak.
- **Native f64/unwrap hygiene (2026-09-20):** `amount.rs` Decimal fee plan; refuse float `amount_satoshi`/`fee_satoshi`; no `to_f64().unwrap_or(0.0)`; StateEngine/map gets typed refuse. `evm_writeback.rs` drops IEEEГ—1e6; display via `from_satoshi_float_inner`. Operator: `.\scripts\verify_native_f64_hygiene.ps1`. **Not** mesh probe / **not** 48h soak.
- **Persist fail-closed (2026-09-20):** hot writes (`save_block` / `persist_block_atomic` / `save_transaction` / chain_storage saves) raise `PersistError` instead of soft `False`. Ops `backup_to` also raises (MED 2026-09-21). Units: `tests/unit/test_persist_fail_closed.py`. Operator: `.\scripts\verify_persist_fail_closed.ps1`. **Not** mesh probe / **not** 48h soak / **not** native unwrap.
- **ADR 0021 wire satoshi cutover (2026-09-20):** P2P `mempool_tx_to_wire` emits canonical `fee_satoshi`/`amount_satoshi`; ABS floats are display dual-write derived from satoshi. Ingest prefers satoshi and refuses `fee_satoshi_mismatch` / `value_satoshi_mismatch`. **MED 2026-09-21:** float-only ingress refused by default (`p2p_mempool_require_wire_satoshi`). Units: `tests/unit/test_adr0021_wire_satoshi_cutover.py`. Industrial gate needles added. **Not** mesh probe / **not** 48h re-soak / **not** native unwrap.
- **Industrial max scan + fail-closed sprout defaults (2026-09-20):** deep stub/float/feature scan вЂ” report [`docs/INDUSTRIAL_MAX_SCAN_2026-09-20.md`](docs/INDUSTRIAL_MAX_SCAN_2026-09-20.md). Landed: PQ boot honesty (no hash-demo), Config/`FeatureFlags` sprout defaults OFF, `node.industrial*.json` bridge/oracles OFF. Follow-ups closed wire-float + persist raise + native f64 hygiene; remaining HIGH honesty items listed in scan. Not soak / not mainnet.
- **Phase 5 lab re-verify (2026-09-20):** `verify_oracle_lab` / `verify_cross_shard_lab` / `verify_bridge_off_lab` all PASS; prod mesh `feature_oracles`/`feature_sharding`/`bridge_enabled` remain false. EXECUTION_ORDER: B3 closed via [`mempool48pass1`](docs/evidence/runs/mempool48pass1/). Not soak / not prod arm / not mainnet.
- **48h mempool+validation STRICT dual-report PASS (2026-09-17в†’19):** evidence [`mempool48pass1`](docs/evidence/runs/mempool48pass1/) вЂ” soak `passed=true` hard_fails=0 mesh_warn=0 ready_only=0 (2589 mesh_ok, warn_lines=31 soft, FullHarnessEvery=6, tip ~35187в†’~44020) + sidecar `passed=true` admit_ok=303 admit_soft=164 refuse_ok=467 refuse_fail=0 admit_fail=0, store=rust demoted=False. Prior FAIL 2026-09-13в†’15 (35) and 6h dual PASS 2026-09-16 remain on record. **Not** mainnet / **not** Hybrid / **not** Long-Range / **not** BLS.
- **6h mempool+validation STRICT PASS (2026-09-16):** dual-report green вЂ” soak `passed=true` hard_fails=0 mesh_warn=0 (351 cycles, tip 30650в†’32008) + sidecar `passed=true` admit/refuse 114/114 fail=0, store=rust demoted=False. AlwaysFullHarness@60s. **Not** 48h / **not** mainnet / **not** Hybrid. Residual: 95Г— soft WARN `p2p=inconsistent` were tip-race labels (mesh always aligned) вЂ” fixed via `tip_skew`/`tip_lagging` split.
- **p2p_sync_status tip honesty (2026-09-16):** `_derive_p2p_sync_status` reserves `inconsistent` for `state_consistent=False`; peer_gapв‰¤2 + consistent в†’ `tip_skew`, gap 3вЂ“20 в†’ `tip_lagging`. health_watch no longer WARN on `tip_skew` (6h soak noise). Unit: `test_derive_p2p_sync_status_helpers`.
- **Hard-analysis follow-ups (2026-09-15):** exclude soft ready_flap/live_fallback tips without head from mesh rows (Height=0 poison); WARN on `p2p=inconsistent`; sidecar demote via probe/ready + stronger timeout classify + refuse timeout 45s. Prior 48h still FINISHED_FAIL (35). Not a PASS; soak not re-started.
- **Residual re-soak landmines closed (2026-09-15):** sidecar reads `store_demoted`/`store_backend`; `stop_soak_monitors` kills sidecar; `check_soak` gates sidecar report + prefers mempool_validation log; Strict live_fallback when ready+status HOL; SkipRebuild hard-fail on mempool soak starter. Hard analysis canvas: prior 48h FAIL cascade after 18:00 under AlwaysFullHarness. Not a 48h PASS; soak not re-started.
- **48h mempool+validation STRICT FINISHED_FAIL triage (2026-09-15):** 35 FAIL = 14Г— tip delta=1 (confirm too short under load) + 17Г— ready_flap 503 (Strict hard-FAIL while `/status` live) + 4Г— harness_timeout (Aligned forced false). Fixes: multi-pass Strict confirm (4Г—3s); ready_flap soft when status OK; harness_timeout keeps tip aligned; long Strict (`Hours>=12`) uses FullHarnessEvery=6 not AlwaysFullHarness; sidecar timeouts soft + backoff, default interval 300s. Prior run: tip 22114в†’30613, 445Г— strict_confirm, sidecar admit 724/1219 вЂ” **not** a 48h PASS claim. Soak not re-started.
- **Mempool miner pack + validation soak prep (2026-09-13):** `Mempool.get_for_block` (nonce-contiguous) wired into live miner/`PBS`/`MEV` via `abs_to_wei` (no `fee*1e9`). Harness uses shared pack. Sidecar `mempool_validation_sidecar.py` + `start_mempool_validation_soak.ps1` ready вЂ” soak **not** started. Not 48h / not wire float cutover.
- **Strict mesh confirm (2026-09-13):** `Test-MeshCycleAligned -Strict` confirms with parallel resnapshot before FAIL (mining-window delta=1 flake). `health_watch` increments `hard_fails` on Strict mesh/harness FAIL. `check_soak` recovers STRICT meta from report when `soak_active.json` gone. First 2h pre48h maxload: `FINISHED_FAIL` (4Г— delta=1 race). Re-run after fix: **FINISHED_PASS** (`fail_lines=0`, 11Г— `strict_confirm=1`, tip +453). Not a 48h claim / not mainnet.
- **Pre-48h max-load 2h STRICT (2026-09-13):** operator script `scripts/start_pre48h_maxload_2h.ps1` вЂ” host gates + mesh rebuild/probe + extreme EVM/HTTP load + 2h STRICT soak (`hard_fails=0` / `mesh_warn=0`). Not a 48h claim / not mainnet.
- **Wave R audit honesty fixes (2026-09-13):** `tx_signer` refuse invented fee `0.001` in hash + ECDSA-missing raise; mempool store ABS from satoshi; AI validator empty avg=0; `/tx/verify` в†’ `valid:null` when unavailable; `verify_wave_r.ps1`. Not soak.
- **Wave Q audit honesty fixes (2026-09-13):** `/sharding/balance` satoshi; `/tx/sign` requires fee (no `0.001` invent); MEV value via money_abs (no `fee*1e9`); wallet ECDSA-missing в†’ RuntimeError; attestation derive/probe в†’ unavailable (P2P strike distinct); `verify_wave_q.ps1`. Not soak.
...
- **Wave N audit honesty fixes (2026-09-13):** native canonicalize/`_sat` via `to_satoshi` (no floatГ—1e6); PoS `consensus_engine` stake int satoshi + finality empty-set no invented denom; multisig `amount_satoshi` + `execution_failed`в†’`success:false`; secp verify exceptionв†’`None` (not False); eth_format corrupt stored rootsв†’null; `verify_wave_n.ps1`. Not soak.
...
- **Wave K audit honesty fixes (2026-09-13):** P2P value gates via satoshi; plasma unsigned refuse; cross-shard debit satoshi; `verify_pre_soak -SkipMesh` в†’ static-only (not pre-soak PASS); smart-account recovery requires guardian_verifier+credential; `verify_wave_k.ps1`. Not soak.
...
- **Ops Console UI (2026-09-13):** same-origin `web/console/` (CSP, path-traversal refuse) вЂ” live `/metrics` tiles + sparklines, under_mesh/mesh honesty, mempool_store, EIP-1193 Absolute chain switch + `eth_sendTransaction`, council watch (778889), Markets panel (exchange clocks, FX converter, crypto/commodities/equities via allowlisted TLS proxy), dark/light theme. `/` в†’ console; `/explorer` в†’ legacy. Grafana panels for `abs_p2p_under_mesh` / sync gap. Not soak / not mainnet.
- **Wave D council + under_mesh DX (2026-09-13):** `verify_council_lab.ps1` (ADR 0022 / Profile C) + pack `councillab1`. Prometheus: `abs_p2p_under_mesh`, `abs_p2p_sync_status`, `abs_p2p_mesh_min_peers`, `abs_p2p_peer_sync_gap` from `/metrics` sync snapshot. Not soak / not prod council mint.- **Wave C soft-WARN / Rocks honesty (2026-09-13):** consistency harness retries peer state_root probe once on timeout/empty (`peer_probe_attempts`); Rocks `pack_*` JSON fallback counted + refused when `ABS_REQUIRE_NATIVE_CRYPTO`; metric `abs_rocksdb_native_pack_fallbacks`. Not soak.
- **Wave B honesty DX (2026-09-13):** `/status` exposes `mempool_store` (backend/demoted/demote_count); prod+require_native demote в†’ `degraded`. COMPAT_MATRIX RPC null-honesty rows relabeled **Supported (Absolute honesty)**. `verify_long_range_lab.ps1` + pack `lrlab1`. Stale Profile F вЂњnext Phase 4вЂќ fixed. Not soak / not prod LR arm.
...
- **EVM depth lab verify pack (2026-09-13):** `verify_evm_depth_lab.ps1` в†’ [`evmlab1`](docs/evidence/runs/evmlab1/). Host labs+unit+gate; mesh optional `-WithMesh`. Not soak / not EVM-only 48h.
...
- **Phase 5 labs PASS (2026-09-13):** Oracle [`oraclelab1`](docs/evidence/runs/oraclelab1/) В· Cross-shard [`shardlab1`](docs/evidence/runs/shardlab1/) В· Bridge OFF [`bridgeoff1`](docs/evidence/runs/bridgeoff1/). Prod flags stay false. Not soak / not L1 cutover.
- **Phase 5.2 Oracle lab PASS (2026-09-13):** `verify_oracle_lab.ps1` 4/4 вЂ” pack [`docs/evidence/runs/oraclelab1/`](docs/evidence/runs/oraclelab1/). Aux SQLite only; `feature_oracles=false` on prod mesh. Not soak / not prod arm.
- **Global R&D audit PASS (2026-09-13):** `verify_global_rd_audit.ps1 -Mode FullLaunch -Rebuild` в†’ **13/13** вЂ” pack [`docs/evidence/runs/adr0021gaudit1/`](docs/evidence/runs/adr0021gaudit1/). Includes ADR 0021 rust families, demote harden, mesh rebuild, probe, EVM smoke, load harness, prepare READY. **Not** 48h soak / not mainnet.
- **Pre-soak harden (2026-09-13):** MempoolStore fault demotes to Python dict (no process crash); `MempoolPort.add_batch` matches Tuple API; TxPipeline kernel/admit exceptions logged. Fee_satoshi dual-write for store/sort landed; wire ABS float still present.
...
- **Phase 3 closed (2026-09-11в†’13):** Experimental prod mesh 48h PASS after EVM preflight вЂ” pack [`docs/evidence/runs/evm48pass1/`](docs/evidence/runs/evm48pass1/) (`passed=true`, `hard_fails=0`, `mesh_warn=0`, tip ~10125в†’~19197). Preflight: `scripts/evm_pre_48h_harness.py` + prod `mem_limit` 2048m. **Not** EVM-only 48h / not mainnet / not BLS.
- **B2 closed (2026-09-09в†’11):** Long-Range **lab** 48h PASS [`docs/evidence/runs/lr48pass1/`](docs/evidence/runs/lr48pass1/) (`hard_fails=0`, `mesh_warn=0`). Prior FAIL `lr48fail1`; intensify `lr2hintensify`. Prod JSON keeps `feature_long_range=false`.
- **B1 closed (2026-09-01в†’03):** libp2p 48h PASS [`docs/evidence/runs/3c801b87/`](docs/evidence/runs/3c801b87/). ADR 0020 Experimental mesh. Hybrid pin untouched.
- Experimental 48h prod-profile soak **PASS** (2026-08-20в†’22, TCP+TLS, `hard_fails=0`, image `0a7932c4` / bake `3c88632`). Packaged at `docs/evidence/runs/0a7932c4/`. Not libp2p cutover. Not Long-Range. Prior FAIL 2026-08-16в†’18 remains on record.
...
- Next: Phase 5+ optional labs, or soak only if explicitly ordered. Not mainnet.
...
- Long-Range (ADR 0017): checkpoint height is unique вЂ” a different hash at the anchor height is `anchor_hash_mismatch` (not a descendant), even if the caller claims shared ancestry; the exact checkpoint hash is `is_anchor` without a parent walk. Soak not run.
- Long-Range (ADR 0017): persist load requires a digest вЂ” omitting it no longer accepts a rewritten height/hash. Load error still fail-closed empty WS (tip-import `ws_no_anchor`), not a dropped gate. Digest-only, not BLS. Soak not run.
...
- EVM: precompiles 0x01вЂ“0x09 on the apply-path nested CALL/STATICCALL hook and `call_contract` (not eth_call only). Identity/sha256 evidenced. Not a geth gas audit. Soak not run.
- EVM: host apply treats `to` in 0x01вЂ“0x09 as a message-call (not CREATE); mempool `_is_evm_deploy_tx` skips precompiles. Soak not run.
- EVM: nested CALL to an empty account (EOA) succeeds with empty returndata and still transfers value. No-code / precompile writeback drops `set_storage` so DELEGATECALL into 0x01вЂ“0x09 cannot wipe the caller. Soak not run.
...
- Slices **AвЂ“CY** (phase 102) behind Cargo `libp2p`
...
- Circuit `/p2p-circuit` never occupies the crate book (CWвЂ“CX)
...
- ADR 0017 Long-Range labs В· ADR 0018 dual-stack adapter В· EVM precompile waves
...
- Wave-7: EVM bn254 ecAdd/ecMul/ecPairing (0x06вЂ“0x08 via `py_ecc`); libp2p discovery stub; `CheckpointStore.apply_latest`
...
- **Public surface polish** вЂ” tighter README (evidence-first hero), ADR index, architecture ADR table 0008вЂ“0016, banner refresh; pin docs to `v1.3.1339-tip-v2-industrial`
- **Audit pin tag `v1.3.1339-tip-v2-industrial`** + [AUDIT_ENGAGEMENT_BRIEF.md](docs/AUDIT_ENGAGEMENT_BRIEF.md) for firm engagement
- **Phase 2 tip-v2 48h soak PASS** (2026-08-05в†’07): `logs/soak_report_tipv2_48h_rerun.json` (`passed=true`, fail=0, mesh_warn=0); evidence `docs/evidence/runs/375d14f/`
...
- Tip-v2 re-smoke + prior Aug 2вЂ“4 48h FAIL (historical only; superseded by Aug 5вЂ“7 PASS)
- `export_audit_pack` / `industrial_gate` recognize tip-v2 evidence under `docs/evidence/runs/`
- Gates load `.env` for ceremony pin; skimmer honesty (AT_A_GLANCE Phase 3вЂ“4)

### Wave D вЂ” bake mesh harden + attestation/rate soft-refuse

- Soft-refuse `attestation_local_height_mismatch`, ingress `rate_limit_*`, and `tip_unknown_parent` (not state_root response codes вЂ” those must still dispatch for wire probe)
- Restore lexicographic dial ownership replace; skip remembering docker IP dial targets
- Canonical bootstrap dials only (mesh-1в†’2/3, mesh-2в†’3, mesh-3 empty) to avoid inverted dual-dial
- Bind bootstrap hostnameв†’peer_id so inbound-IP peers cover seeds
- `PeerConnection.close` cancels send worker (no asyncio вЂњTask was destroyedвЂќ storm)
...
- Priority P2P sends (state_root / get_block / вЂ¦) bypass the gossip send queue so attestations cannot starve wire probes
- `health_watch` truncates its log at start (soak FAIL counts are session-scoped)
- Image-baked readyГ—3 + probe on `abs-blockchain-prod:local`; short 2h soak evidence under commit SHA

### Wave A вЂ” TLS dual-dial ownership + ready gate

- Canonical peer dial ownership (lexicographic node_id): keep one live registration under Aв†”B dual dial
- Native TLS `close_notify` before TCP shutdown
- Bootstrap/discovery dials coalesce via `_schedule_connect`
- `verify_p2p_ci` asserts `/health/ready` PASS Г—3; CI artifact `mesh-ready-gate`

### Wave B вЂ” evidence + secrets/CI harden
...
### Wave C вЂ” tip+apply satoshi integer cutover
...
### Mesh вЂ” genesis artifact + tip height-0 + P2P IO

- Shared ceremony genesis JSON (`sync/genesis_artifact.py`, `GENESIS_ARTIFACT_PATH`) so followers import leader #0 + founder вЂ” no divergent local mint
- TipSafety / STATUS / handshake treat height **0** as present; empty genesis import skips reward mutation
- Native P2P short-poll reads (`set_read_timeout_only`) вЂ” avoid write-starve under long blocking reads
...
### P2P вЂ” mesh soft-refuse (probe 503)
...
- Probe FAIL path when peers=0 after bans в†’ `peers_alive=false`

### Architecture вЂ” ADR 0016 feature sprouts
...
- `features/nft_ports.py`; NFT в†’ `app-profile`; mesh JSON freeze `feature_*=false`
- Prod compose healthcheck `/health/live` (not `/ready`) вЂ” avoid depends_on в†” peer-quorum chicken-egg
- Serialize `P2PRateLimitTable` + per-conn native IO locks
- Docs: `docs/sprouts/*`; architecture mermaid + honest Proven table

### Docs / GitHub UX

- First-screen honesty: chain sync **Proven**, `/health/ready` **Partial**; ADR **0001вЂ“0016**

## [1.3.1338-deterministic-core] вЂ” 2026-07-30
...
- ADR 0010 BridgePort В· ADR 0011 QueryFacade В· ADR 0012 Chaos В· ADR 0014 Graceful shutdown

## [1.3.205] вЂ” 2026-07-26

### Mempool вЂ” unparseable-nonce refuse

- P2P refuses junk / Inf `nonce` before `validate_transaction` (`nonce_unparseable`)
- Soft DoS honesty вЂ” no OverflowError into ingest; complements nonce_negative / nonce_too_high
- Honesty: not tip proof / Long-Range / libp2p / public mainnet / account-nonce sequencing

## [1.3.204] вЂ” 2026-07-26

### Mempool вЂ” unparseable-value refuse

- P2P refuses junk `value` / `amount` before `validate_transaction` (`value_unparseable`)
- Soft DoS honesty вЂ” no TypeError into ingest; complements value_negative / value_non_finite
- Honesty: not tip proof / Long-Range / libp2p / public mainnet / amount-cap economics

## [1.3.203] вЂ” 2026-07-26

### Mempool вЂ” unparseable-gas refuse

- P2P refuses Inf/junk `gas` before `validate_transaction` (`gas_unparseable`)
- Soft DoS honesty вЂ” no OverflowError into ingest; complements gas_negative / gas_too_high
- Honesty: not tip proof / Long-Range / libp2p / public mainnet / Rust gas PQ

## [1.3.202] вЂ” 2026-07-26

### Mempool вЂ” max-value refuse

- P2P refuses oversized wire `value` before `validate_transaction` (`value_too_high`)
- Soft DoS honesty вЂ” default 221M ABS (aligns `max_supply`); fantasy amounts fail before DB
- Honesty: not tip proof / Long-Range / libp2p / public mainnet / full tokenomics

## [1.3.201] вЂ” 2026-07-26

### Mempool вЂ” max-fee refuse

- P2P refuses oversized wire `fee` before `validate_transaction` (`fee_too_high`)
- Soft DoS honesty вЂ” default 1e9 ABS; complements `fee_too_low`
- Honesty: not tip proof / Long-Range / libp2p / public mainnet / fee-market

## [1.3.200] вЂ” 2026-07-26

### Mempool вЂ” max-nonce refuse

- P2P refuses oversized wire `nonce` before `validate_transaction` (`nonce_too_high`)
- Soft DoS honesty вЂ” default 1e12 (MAX_P2P_HEIGHT-style); fantasy nonces fail before DB
- Honesty: not tip proof / Long-Range / libp2p / public mainnet / nonce-window

## [1.3.199] вЂ” 2026-07-26

### Mempool вЂ” max-to refuse

- P2P refuses oversized wire `to` before `validate_transaction` (`to_too_large`)
- Soft DoS honesty вЂ” mirrors `from_too_large`; shares `max_addr_chars` / `MAX_P2P_ADDR_LEN`
- Honesty: not tip proof / Long-Range / libp2p / public mainnet / checksum

## [1.3.198] вЂ” 2026-07-26

### Mempool вЂ” max-from refuse

- P2P refuses oversized wire `from` before `validate_transaction` (`from_too_large`)
- Soft DoS honesty вЂ” default 128 chars, aligns Rust `MAX_P2P_ADDR_LEN`
- Honesty: not tip proof / Long-Range / libp2p / public mainnet / checksum

## [1.3.197] вЂ” 2026-07-26

### Mempool вЂ” max-hash refuse

- P2P refuses oversized wire `hash` before `validate_transaction` (`hash_too_large`)
- Soft DoS honesty вЂ” default 128 chars, aligns Rust `MAX_P2P_HASH_LEN`
- Honesty: not tip proof / Long-Range / libp2p / public mainnet / hashв†”body bind

## [1.3.196] вЂ” 2026-07-26

### Mempool вЂ” empty-hash refuse

- P2P refuses empty/whitespace wire `hash` before `validate_transaction` (`hash_empty`)
- Soft DoS honesty вЂ” empty hash skipped `duplicate_tx`; this closes that gap
- Honesty: not tip proof / Long-Range / libp2p / public mainnet / hashв†”body bind

## [1.3.195] вЂ” 2026-07-26

### Mempool вЂ” empty-to refuse

- P2P refuses empty/whitespace `to` before `validate_transaction` (`to_empty`)
- Soft DoS honesty вЂ” mirrors `from_empty`; cheaper than `missing_address`
- Honesty: not tip proof / Long-Range / libp2p / public mainnet / contract-create

## [1.3.194] вЂ” 2026-07-26

### Mempool вЂ” non-finite fee refuse

- P2P refuses NaN/Inf `fee` before `validate_transaction` (`fee_non_finite`)
- Soft DoS honesty вЂ” complements `fee_negative` (NaN/Inf slip past `< 0`)
- Honesty: not tip proof / Long-Range / libp2p / public mainnet

## [1.3.193] вЂ” 2026-07-26

### Mempool вЂ” non-finite value refuse

- P2P refuses NaN/Inf `value` before `validate_transaction` (`value_non_finite`)
- Soft DoS honesty вЂ” complements `value_negative` (NaN/Inf slip past `< 0`)
- Honesty: not tip proof / Long-Range / libp2p / public mainnet

## [1.3.146] вЂ” 2026-07-26

### P2P вЂ” catch-up tip probe + headв†”height bind
...
## [1.3.145] вЂ” 2026-07-26

### P2P вЂ” peer score quality (strikes + import fails)
...
## [1.3.144] вЂ” 2026-07-26

### P2P вЂ” solicit-armed mempool shell (skip unsolicited ECDSA)
...
## [1.3.143] вЂ” 2026-07-26

### Mempool вЂ” cheap refuse + new_tx primary rate
...
## [1.3.142] вЂ” 2026-07-26

### Tests вЂ” Standard pytest solicit-only needle
...
## [1.3.141] вЂ” 2026-07-26

### Sync вЂ” same-height state match is wire-only
...
## [1.3.140] вЂ” 2026-07-26

### Sync вЂ” never invent peer.head from local blocks
...
## [1.3.139] вЂ” 2026-07-26

### P2P вЂ” catch-up requires peer.head
...
## [1.3.138] вЂ” 2026-07-26

### P2P + ops вЂ” solicit-only state_root + ceremony status

- Unsolicited state_root_response struck; never flips consistency
- `ceremony_status.py` in `check_all` вЂ” honest readiness, never invents pin
- Honesty: not tip proof / mainnet / completed audit

## [1.3.137] вЂ” 2026-07-26

### P2P вЂ” attestation local-head + solicit-only block responses

- Known target_hash в‡’ target_height must match local header
...
## [1.3.136] вЂ” 2026-07-26

### P2P вЂ” soft attestation slot-ahead ownership gate

- Far-ahead attestations refused before LMD apply/relay
- `p2p_max_attestation_slot_ahead` (default matches height ahead window)
- Honesty: not attestationв†”head crypto / tip proof / libp2p / mainnet

## [1.3.135] вЂ” 2026-07-26

### P2P вЂ” local state_root consistency + tip ownership completion

- Known local header в‡’ peer state_root must match; historical expected_head fixed
...
## [1.3.134] вЂ” 2026-07-26

### P2P вЂ” soft NEW_BLOCK height-ahead ownership gate
...
## [1.3.133] вЂ” 2026-07-26

### P2P вЂ” authenticated bootstrap seed pins
...
## [1.3.132] вЂ” 2026-07-25

### P2P вЂ” resilient bootstrap redial
...
## [1.3.131] вЂ” 2026-07-25

### P2P вЂ” solicit-only mempool + status height ahead cap
...
## [1.3.129] вЂ” 2026-07-25

### P2P вЂ” outbound state_root height honesty

- Historical state_root probes return that blockвЂ™s root/hash (never mislabeled tip)
- Ahead/missing requests refused; unsolicited responses cannot inflate peer.height
- Honesty: not tip proof / fork-choice / libp2p / mainnet

## [1.3.128] вЂ” 2026-07-25

### P2P вЂ” discovery dialability + soft heightв†”head binding
...
## [1.3.127] вЂ” 2026-07-25

### P2P вЂ” request-bound state_root_response height gate
...
## [1.3.126] вЂ” 2026-07-25

### P2P вЂ” request-bound singular block response hash gate
...
## [1.3.125] вЂ” 2026-07-25

### P2P вЂ” request-bound blocks response + prod native shell fail-closed
...
## [1.3.124] вЂ” 2026-07-25

### P2P вЂ” native status.head_hash digest semantic gate

- Loop-shell: non-empty `status.head_hash` must be 32-byte hex (optional `0x`)
- Empty / null / non-object keepalives remain OK
- Honesty: heightв†”hash binding / peer auth / full message-loop not claimed

## [1.3.123] вЂ” 2026-07-25

### P2P вЂ” native state_root_response digest semantic gate
...
## [1.3.122] вЂ” 2026-07-25

### P2P вЂ” native singular block response hash semantic gate

- Loop-shell verifies non-null `block` payloads (canonical hash); `null` = not-found OK
- Completes new_block в†’ blocks в†’ block hash ingress sequence
- Honesty: request correlation / import / full message-loop not claimed

## [1.3.121] вЂ” 2026-07-25

### P2P вЂ” native blocks batch hash semantic gate + Makefile UX
...
## [1.3.120] вЂ” 2026-07-25

### P2P вЂ” native new_block canonical-hash semantic gate

- Loop-shell rejects `new_block` when claimed hash в‰  canonical recompute
...
## [1.3.119] вЂ” 2026-07-25

### P2P вЂ” native mempool batch signature semantic gate
...
## [1.3.118] вЂ” 2026-07-25

### P2P вЂ” native new_tx signature semantic gate
...
## [1.3.117] вЂ” 2026-07-25

### P2P вЂ” native attestation semantic gate
...
## [1.3.116] вЂ” 2026-07-25

### P2P вЂ” native message-loop event shell
...
## [1.3.115] вЂ” 2026-07-25

### P2P вЂ” native handshake policy fuse + Max-gate fixes
...
## [1.3.114] вЂ” 2026-07-25

### P2P вЂ” prod-mandatory native transport
...
## [1.3.113] вЂ” 2026-07-25

### P2P вЂ” native handshake payload gate

- Fail-closed handshake/ack shape check on `handshake_roundtrip` (`bad_handshake_payload`)
- Chain-id / TLS / policy remain Python
- Honesty: still not full message-loop / libp2p

## [1.3.112] вЂ” 2026-07-25

### P2P вЂ” native cross-shard shape gates
...
## [1.3.111] вЂ” 2026-07-25

### P2P вЂ” native state-root shape gates
...
## [1.3.110] вЂ” 2026-07-25

### P2P вЂ” native peer discovery shape gates

- Fail-closed `peers` / `validator_register` on native read
- Strike reasons: `bad_peers_list` / `bad_validator_register`
- Honesty: still not full message-loop / libp2p

## [1.3.109] вЂ” 2026-07-25

### P2P вЂ” native singular block payload gate
...
## [1.3.108] вЂ” 2026-07-25

### P2P вЂ” native tx gossip shape gates
...
## [1.3.107] вЂ” 2026-07-25

### P2P вЂ” native block fetch shape gates
...
## [1.3.106] вЂ” 2026-07-25

### P2P вЂ” native block sync shape gates
...
## [1.3.102] вЂ” 2026-07-25

### P2P вЂ” native I/O timeout config
...
## [1.3.101] вЂ” 2026-07-25

### P2P вЂ” native batch/chunk config knobs

- Config `p2p_native_read_batch` / `write_batch` / `read_chunk` (+ env)
- Shared Rust clamp helpers; status + Prometheus gauges
- Honesty: still not full message-loop / libp2p

## [1.3.100] вЂ” 2026-07-25

### P2P вЂ” native housekeeping payload gate
...
## [1.3.99] вЂ” 2026-07-25

### P2P вЂ” native keepalive consume (pong + touch)
...
## [1.3.98] вЂ” 2026-07-25

### P2P вЂ” native auto-pong keepalive
...
- Honesty: keepalive only вЂ” still not full message-loop / libp2p

## [1.3.97] вЂ” 2026-07-25

### P2P вЂ” native peer cert CN/SAN identities
...
## [1.3.96] вЂ” 2026-07-25

### P2P вЂ” native handshake_roundtrip I/O fuse
...
## [1.3.95] вЂ” 2026-07-25

### P2P вЂ” native write_messages / write_payloads batch
...
## [1.3.94] вЂ” 2026-07-25

### P2P вЂ” native read_messages batch pump
...
## [1.3.93] вЂ” 2026-07-25

### P2P вЂ” native write_message pump (encode + write)
...
## [1.3.92] вЂ” 2026-07-25

### P2P вЂ” native read_message pump (frame + wire parse)
...
## [1.3.91] вЂ” 2026-07-25

### P2P вЂ” native rustls TLS on transport slice
...
## [1.3.90] вЂ” 2026-07-25

### P2P вЂ” native plain-TCP transport slice
...
## [1.3.89] вЂ” 2026-07-25

### P2P вЂ” Sybil / Eclipse hardening
...
## [1.3.88] вЂ” 2026-07-25

### Native вЂ” P2P kernel fuzz / smoke
...
- Honesty: fuzz в‰  full audit; TCP/message loop still Python; not public mainnet

## [1.3.87] вЂ” 2026-07-25

### P2P вЂ” unified native egress prepare
...
## [1.3.86] вЂ” 2026-07-25

### P2P вЂ” native NDJSON line framer

- `P2PLineFramer`: buffered chunk в†’ complete `\n` lines; fail-closed oversize before newline
- Wired into `PeerConnection._read_wire_line` (chunked `read` + framer)
- Honesty: not full Rust transport; TCP/message loop still Python

## [1.3.85] вЂ” 2026-07-25

### P2P вЂ” outbound egress bandwidth (cost-weighted)
...
## [1.3.84] вЂ” 2026-07-25

### EVM вЂ” inline CREATE в†’ save_account writeback journal
...
## [1.3.83] вЂ” 2026-07-25

### EVM вЂ” inline value в†’ satoshi writeback journal
...
## [1.3.82] вЂ” 2026-07-25

### EVM вЂ” CREATE/CREATE2 eligible init with RETURN runtime
...
## [1.3.81] вЂ” 2026-07-25

### EVM вЂ” inline CREATE2 (empty / STOP init)

- CREATE2 (`0xF5`) with empty/STOP init runs in Rust when `bridge_state.codes` present
- Address: EIP-1014 by default; `create2_eip1014=false` в†’ Absolute legacy seed
...
## [1.3.80] вЂ” 2026-07-25

### EVM вЂ” inline simple CREATE (empty / STOP init)
...
- Value transfer fail-closed via balances; insufficient в†’ push 0 without hook
...
## [1.3.79] вЂ” 2026-07-25

### EVM вЂ” CALLCODE value ownership (fail-closed balances)
...
- Insufficient balance в†’ CALLCODE fails without child exec / without Python hook
- Missing balances map в†’ fall through to Python hook
- Honesty: CREATE frames / DB satoshi journal ownership still not claimed

## [1.3.78] вЂ” 2026-07-25

### P2P вЂ” per-peer bandwidth / cost-weighted ingress budget

- `P2PRateLimitTable` byte window + `ingress_cost_units` (blocks/mempool Г—2)
...
## [1.3.77] вЂ” 2026-07-25

### P2P вЂ” Rust ingress data plane (wire + rate) + connection governor

- `p2p_ingress_admit`: one native path for decode в†’ type allowlist в†’ primary/exempt rate
...
## [1.3.76] вЂ” 2026-07-25

### EVM вЂ” value-transfer CALL ownership (fail-closed balances)

- Inline CALL with `value > 0` when `bridge_state.balances` is present
- Fail-closed debit (no silent clamp); insufficient в†’ CALL fails without child exec
- Revert restores balance snapshot; success keeps transfer + storage
- Missing balances map в†’ fall through to Python hook (real DB path)
- Honesty: CALLCODE value / DB-backed satoshi writeback still via adapter journal

## [1.3.75] вЂ” 2026-07-25

### EVM вЂ” multi-depth value=0 CALL frames (Priority 38)
...
## [1.3.74] вЂ” 2026-07-25

### EVM вЂ” value=0 CALL/STATICCALL inline leaf (Priority 38)
...
## [1.3.73] вЂ” 2026-07-25

### Load вЂ” apply-queue priority lanes
...
## [1.3.72] вЂ” 2026-07-25

### P2P вЂ” sync admission + outbound honesty (close v1.3.66 debt)
...
## [1.3.71] вЂ” 2026-07-25

### EVM вЂ” in-Rust inline leaf frame (Priority 37)
...
## [1.3.70] вЂ” 2026-07-25

### EVM вЂ” recursive frame correctness (arena sync)
...
## [1.3.69] вЂ” 2026-07-25

### Load вЂ” block-scoped sat session for mixed apply
...
- `scripts/verify_industrial_waves.py` вЂ” full checklist for waves 1.3.65вЂ“1.3.69

## [1.3.68] вЂ” 2026-07-25

### Bridge вЂ” semantic event bind + fail-closed debit

- `try_debit_satoshi` вЂ” underflow raises (no silent clamp)
...
## [1.3.67] вЂ” 2026-07-25

### EVM вЂ” tx writeback journal + Rust storage arena
...
## [1.3.66] вЂ” 2026-07-25

### Load вЂ” apply backpressure + tip O(1) + P2P coalesce
...
## [1.3.65] вЂ” 2026-07-25

### Security вЂ” L1 fail-closed hardening
...
## [1.3.64] вЂ” 2026-07-25

### Added вЂ” Rocks batch writeback account preload

- `RocksEngine.get_account_rows` вЂ” batch load account JSON rows
- `RocksChainStore.load_writeback_accounts` / Hybrid delegate
- Adapter preloads touched accounts via Rocks before native apply

## [1.3.63] вЂ” 2026-07-25

### Added вЂ” unified writeback bundle (accounts + logs)

- `RocksEngine.commit_writeback_bundle` вЂ” single WriteBatch for accounts + EVM logs
...
## [1.3.62] вЂ” 2026-07-25

### Added вЂ” store-lock Rocks writeback commit

- `RocksEngine.commit_account_rows` вЂ” native batch account put
- `RocksChainStore.commit_writeback_accounts` under `_write_lock`
- Adapter uses store-lock commit after `evm_apply_writeback_ops`

## [1.3.61] вЂ” 2026-07-25

### Added вЂ” native writeback apply (in-memory)

- `evm_apply_writeback_ops` вЂ” apply planned ops to accounts map in Rust
- Adapter commits via Python DB after native apply

## [1.3.60] вЂ” 2026-07-25

### Added вЂ” CREATE writeback ops (Rust planner)

- `evm_plan_create_writeback` вЂ” `save_account` + optional `transfer_value`
- Adapter CREATE path applies via shared writeback ops (no double-credit)

## [1.3.59] вЂ” 2026-07-25

### Added вЂ” nested CALL writeback ops (Rust planner)

- `evm_plan_nested_call_writeback` вЂ” concrete `ops[]` with resolved addresses
- Adapter applies ops via Python DB (`_apply_nested_writeback_ops`)

## [1.3.58] вЂ” 2026-07-25

### Added вЂ” native account view decode (nested CALL preload)
...
## [1.3.57] вЂ” 2026-07-25

### Added вЂ” LOG/CALL/CREATE host bodies in Rust
...
## [1.3.56] вЂ” 2026-07-25

### Added вЂ” nested CALL/CREATE/LOG host frame (Rust runner + bridge)

- `evm_run_nested_host_frame` вЂ” child frame with runtime `host_bridge` (CALL/CREATE/LOG)
...
## [1.3.55] вЂ” 2026-07-25

### Added вЂ” nested CALL native bridge surface (BALANCE/EXTCODE*)

- `evm_bytecode_is_nested_native_eligible` вЂ” allows bridge ops, rejects recursive CALL/CREATE/LOG
- Nested frame `allow_bridge=True`; adapter keeps host_context bridge hooks

## [1.3.54] вЂ” 2026-07-25

### Added вЂ” EVM/mempool high-load harness

- `scripts/evm_mempool_load_harness.py` вЂ” concurrent mempool enqueue + ChainApplyQueue forge (simple+EVM), not `/health/live`

## [1.3.53] вЂ” 2026-07-25

### Added вЂ” apply isolation metrics + dedicated sync executor
...
## [1.3.52] вЂ” 2026-07-25

### Added вЂ” serial ChainApplyQueue (mine + import)
...
## [1.3.51] вЂ” 2026-07-25

### Changed вЂ” P2P/sync import off the asyncio event loop
...
## [1.3.40] вЂ” 2026-07-21

### Added вЂ” native eth raw tx decode kernel
...
## [1.3.39] вЂ” 2026-07-21

### Added вЂ” native FFG finality + slashing conflict kernels
...
## [1.3.38] вЂ” 2026-07-21

### Added вЂ” native GHOST + simple block apply/replay
...
## [1.3.37] вЂ” 2026-07-21

### Hardened вЂ” bridge L1-proof / light / PBS / AI honesty
...
## [1.3.36] вЂ” 2026-07-21

### Hardened вЂ” WASM/finality/reorg/RANDAO honesty + atomic chain backup replace
...
## [1.3.35] вЂ” 2026-07-21

### Hardened вЂ” MiniVM/ZK/Lightning/DAO honesty + relayer status

- MiniVM feature-gated; Lightning direct-only; unsigned DAO vote blocked in prod
- ZK no invented validity / no GET private keys; bridge_relayer_live в‰  binary smoke

---

## [1.3.34] вЂ” 2026-07-21

### Hardened вЂ” Rust L1 lock verify / receipt status / event-log address / atomic debit
...
## [1.3.33] вЂ” 2026-07-21

### Hardened вЂ” bridge event replay / atomic credit / plasma force / smart-accounts
...
## [1.3.32] вЂ” 2026-07-21

### Hardened вЂ” L1 receipt status / EVM static / NFT-PQ-will-multisig honesty
...
## [1.3.31] вЂ” 2026-07-21

### Hardened вЂ” oracle quorum / sync finally / peer fork / bridge-MEV-AI-will
...
## [1.3.30] вЂ” 2026-07-21

### Hardened вЂ” ready/WS / feature init / bridge proof / L2 missing / storage / consensus
...
## [1.3.29] вЂ” 2026-07-21

### Hardened вЂ” topology/prod / filters / migrate / SQLite features / metrics-WS / tip
...
## [1.3.28] вЂ” 2026-07-21

### Hardened вЂ” mining/status / WS-P2P send / API missing / clone / SQLite / amount
...
## [1.3.27] вЂ” 2026-07-21

### Hardened вЂ” Rocks NFT/EVM decode / catch_up gather / IMS missing / get_meta
...
## [1.3.26] вЂ” 2026-07-21

### Hardened вЂ” remaining gather / Rocks mutate / attestation errors / BlockBuilder
...
## [1.3.25] вЂ” 2026-07-21

### Hardened вЂ” supply canonical / Rocks point-gets / broadcast_fail / core_real
...
## [1.3.24] вЂ” 2026-07-21

### Hardened вЂ” Core engines prod / status wire / IMS canonical / Rocks meta-tx
...
## [1.3.23] вЂ” 2026-07-21

### Hardened вЂ” P2P bind / ready wire / status degraded / peers mining
...
## [1.3.22] вЂ” 2026-07-21

### Hardened вЂ” Rocks decode / topology / SyncEngine prod / eth_mining mesh
...
## [1.3.21] вЂ” 2026-07-21

### Hardened вЂ” sync_state / mesh / bridge-L1 / ready peer_count honesty
...
- L1 unconfigured / bridge disabled в†’ ok=False; ready peer_count probe failure fail-closed

---

## [1.3.20] вЂ” 2026-07-21

### Hardened вЂ” Proxy CORS, receipt omitв†’0, ready/sync honesty

- CORS miss never echoes allowlist; empty list never promotes to `*`
- Ready with peers requires state_consistent; SyncEngine missing в†’ p2p_fallback fail-closed
- Receipt status omit/unknown в†’ 0x0

---

## [1.3.19] вЂ” 2026-07-21

### Hardened вЂ” Sync incomplete, CORS miss, repair success, receipt fail-closed
...
## [1.3.18] вЂ” 2026-07-21

### Hardened вЂ” fast_sync honesty, ready DB probe, Rocks TX iter

- fast_sync: tip-match / no-new / after-import в†’ sync_state(); peer-loss clears stale probed-ok
- Rocks _iter_transaction_rows: warn + json_decode_failures; /health/ready db_probe + db_probe_error

---

## [1.3.17] вЂ” 2026-07-21

### Hardened вЂ” Never-probed wire honesty

- Solo/deferred sync_state leaves wire_probe_ok=None (never probed в‰  probed-ok)
...
## [1.3.16] вЂ” 2026-07-21

### Hardened вЂ” Shared SyncEngine, unsolicited state_root honesty, probe/sqlite alerts
...
## [1.3.15] вЂ” 2026-07-21

### Hardened вЂ” Sync/RPC honesty, SQLite metrics engine, compose freeze
...
## [1.3.14] вЂ” 2026-07-21

### Hardened вЂ” SQLiteв†”Rocks reorg parity, L1 probe honesty, slash/CORS

- SQLite reorg purges evm_logs/tx_prop; truncate_blocks_above = full chain truncate
- Rocks corrupt TX/prop keys deleted on reorg; L1 unprobed в‰  ok
- slash_validator в†’ DB fail-loud; REST CORS empty в‰  *

---

## [1.3.13] вЂ” 2026-07-21

### Hardened вЂ” Rocks/CORS/TLS overlay honesty, fail-closed state defaults
...
## [1.3.12] вЂ” 2026-07-21

### Hardened вЂ” Wire-probe fail-closed, import/sync honesty, ready/compose freeze

- Empty/timeout state_root wire probe with peers в†’ inconsistent (not silent green)
...
- Single-node composeв†”JSON freeze; `.env.example` documents chain_id 778888

---

## [1.3.11] вЂ” 2026-07-21

### Hardened вЂ” Sync consistency metrics, loop/import honesty, deploy freezes
...
## [1.3.10] вЂ” 2026-07-21

### Hardened вЂ” Semantic peer-tx honesty, mesh Redis JSON, compose freeze

- Peer tx semantic/mempool rejects в†’ warn + `peer_tx_reject` counter; gossip strikes `bad_peer_tx`
- `abs_p2p_peer_tx_reject_total` metric + alert/Grafana panel
- Mesh JSON redis parity with compose; industrial_gate composeв†”JSON numeric freeze
- `post_soak_verify` includes `k8s_prod_gate`

---

## [1.3.09] вЂ” 2026-07-21

### Hardened вЂ” Pre-ban strike logs, loop honesty, k8s embed freeze

- `_strike_peer_sync` warns on every pre-ban strike (`strike N/M`)
- `_catch_up_loop` STATUS send return в†’ `peer_status_send_fail`; maintenance/catch_up fails в†’ warning + ops counters
- `k8s_prod_gate`: embedded ConfigMap JSON в‰Ў `node.prod.k8s.json`
...
## [1.3.08] вЂ” 2026-07-21

### Hardened вЂ” Swiss-watch ops honesty, JSON parity, EVM native fail-closed

- Fix dead `peer_status_send_fail` (check `Peer.send` return); mid-session handshake warn logs
- Strike-backed rejects + attestation sign fail в†’ warning; `abs_p2p_attestation_local_fail_total`
...
## [1.3.07] вЂ” 2026-07-21

### Hardened вЂ” ops_errors metrics, mid-session handshake reject, prod rate hard errors

- Prometheus: `abs_p2p_peer_send_fail_total`, `abs_p2p_ops_errors{kind=вЂ¦}` + alert/panel
- Mid-session `handshake`/`handshake_ack` в†’ strike `mid_session_handshake` (not rate-exempt)
- industrial_gate: prod JSON `p2p_max_messages_per_sec > 0` hard error; freeze new metric needles
- `.env.example`: `P2P_HOST`

---

## [1.3.06] вЂ” 2026-07-21

### Hardened вЂ” CI final_audit blocking, housekeeping gates, status honesty
...
## [1.3.05] вЂ” 2026-07-21

### Hardened вЂ” Rate-limit strikes, P2P knob parity, Rocks alerts

- P2P: rate-limit excess в†’ strike/`rate_limit_exceeded`; unexpected recv I/O в†’ `WireReject(recv_error)`
...
## [1.3.04] вЂ” 2026-07-21

### Hardened вЂ” Wire-reject strikes, P2P alerts, Rocks deploy parity

- P2P: malformed/oversized lines в†’ `WireReject` + strike/`shape_rejects` (no silent disconnect)
...
## [1.3.03] вЂ” 2026-07-21

### Hardened вЂ” Observability + ceremony UX + Rocks env in deploy
...
## [1.3.02] вЂ” 2026-07-21

### Hardened вЂ” Post-soak industrial polish (Swiss-watch pass)
...
- node_version default в†’ `1.3.02-industrial`; `scripts/post_soak_verify.py` entry

---

## [1.3.01] вЂ” 2026-07-21

### Hardened вЂ” Soak evidence strict mode, CI audit pack, devnet manifest fail-loud
...
## [1.3.00] вЂ” 2026-07-21

### Hardened вЂ” Audit pack encoding snapshot, founder pin fail-loud
...
## [1.2.99] вЂ” 2026-07-21

### Hardened вЂ” State-root encoding honesty API, harness check, evidence stamp
...
## [1.2.98] вЂ” 2026-07-21

### Hardened вЂ” AI/MEV/PQ probes, security-audit bridge gate, main fail-loud
...
## [1.2.97] вЂ” 2026-07-21

### Hardened вЂ” Lightning/ZK probes, CI bridge gate, audit pack
...
## [1.2.96] вЂ” 2026-07-21

### Hardened вЂ” evidence bridge gate, WASM/Plasma probes, release stamp
...
## [1.2.95] вЂ” 2026-07-21

### Hardened вЂ” bridge OFF audit gate, K8s TLS merge, consensus import errors
...
## [1.2.94] вЂ” 2026-07-21

### Hardened вЂ” K8s per-pod TLS, supply/repair fail-loud, bridge audit checklist
...
## [1.2.93] вЂ” 2026-07-21

### Hardened вЂ” API repair fail-loud, verify_p2p strict skips, cert-manager per-pod
...
## [1.2.92] вЂ” 2026-07-21

### Hardened вЂ” mining fail-loud, state_root encoding scaffold

- Mining loop: PBS/MEV/shard/epoch/light-client errors logged
- `runtime/state_root_encoding.py` вЂ” v1 active, v2 satoshi scaffold blocked
- `/status` exposes `state_root_policy.encoding`
- K8s `cert-manager-p2p.example.yaml`; industrial_gate mining log checks

---

## [1.2.91] вЂ” 2026-07-21

### Hardened вЂ” P2P ops fail-loud, K8s TLS mesh, CI skip policy
...
## [1.2.90] вЂ” 2026-07-21

### Hardened вЂ” status honesty + mesh Redis validate
...
## [1.2.89] вЂ” 2026-07-21

### Hardened вЂ” mesh Redis + auth + honesty
...
## [1.2.88] вЂ” 2026-07-21

### Hardened вЂ” soak / rate limit / TLS
...
## [1.2.87] вЂ” 2026-07-21

### Hardened вЂ” P2P

- TLS enabled в‡’ always `CERT_REQUIRED` (removed `CERT_NONE` path)
...
### Hardened вЂ” L1 bridge tooling (bridge remains OFF)

- Atomic `save_l1_queue`; fail-loud `get_contract_code` on RPC errors
- Cutover: relayer / L1 probe exceptions в†’ errors
...
## [1.2.85] вЂ” 2026-07-21

### Proven (ops)

- **48h prod mesh soak PASS** (2026-07-19 07:02 в†’ 2026-07-21 07:03): `fail_lines=0`, `hours_requested=48`
- `industrial_gate --min-soak-hours 48` OK В· `testnet_readiness -MinSoakHours 48` OK
- Strict pre-rescore report kept (`mesh_warn=11`, all height delta в‰¤1); rescored `passed=true`

### Added / changed

- **`docker-compose.prod.3node.yml`**: json-file log rotation `50m` Г— `3` (soak-safe vs Docker VM disk fill)
- **`health_watch.ps1`**: mesh align allows height delta в‰¤1; tip hash check only when heights equal
...
- **prod_gate:** forbid `allow_state_root_rewrite=true`; mesh1 peersв‰Ґ1; mesh2/3 `follower_genesis_sync`
...
- **`DatabaseStateAdapter`**: `get_balance_satoshi` via `canonical_balance_satoshi` (no floatГ—1e6)
...
- Float ABS column retained for compatibility вЂ” not yet dropped

---

## [1.2.79] вЂ” 2026-07-17

### Fixed / hardened (core РґРѕРІРѕРґРєР° вЂ” no new features)
...
- **IMS sync:** `except: pass` on ImmutableState apply в†’ fail-loud in prod
...
## [1.2.78] вЂ” 2026-07-17

### Added

- **`scripts/export_audit_pack.py`** / **`.ps1`** вЂ” soak-safe static audit pack (gates, docs, soak artifacts, zip + `manifest.json`); never restarts prod mesh
- **`external_audit_tracker`** вЂ” `-SyncAutomated` / `-ShowAutomated`, `--evidence-url` / `--evidence-note` on `--set`
- **`tests/unit/test_export_audit_pack.py`**

### Fixed

- **`prepare_48h_soak.ps1`** вЂ” PowerShell parse error from Unicode em-dash
- **Ops PS1 strings** вЂ” ASCII hyphens in `bridge_cutover_evidence_suite`, `docker_devnet`, `reset_genesis`, `setup_prod_env`

### Changed

- **`industrial_gate.ps1`** вЂ” forwards `-MinSoakHours`, `-CeremonyDir`, `-RequireCeremonyPin`, `-Json`
- **`restart_soak_prod_mesh.ps1`** вЂ” default log `logs/soak_48h_v1.2.77.log`
- **Docs** вЂ” honest 48h soak status (RUNNING since 2026-07-17, not PASS); EVIDENCE_MATRIX, MAINNET_GAP, PUBLIC_TESTNET, REPO_PROFILE

---

## [1.2.77] вЂ” 2026-07-14

### Fixed

- **P2P rate limit** вЂ” exempt `new_block`, `get_block`, `get_blocks`, `new_tx`, and mempool sync types from per-peer 500/s throttle so prod mesh catch-up no longer drops consensus traffic from the leader (`docker-prod-mesh-1`)

### Changed

- **`scripts/industrial_gate.py`** вЂ” stricter check that sync wire types stay rate-limit exempt
- **`tests/unit/test_p2p_industrial.py`** вЂ” expanded sync exempt coverage

---

## [1.2.76] вЂ” 2026-07-14

### Fixed

- **`docker_prod_3node.ps1`** вЂ” PowerShell parse error (Unicode em-dash in TLS verify catch block)
- **`gen_p2p_mesh_tls.py`** вЂ” Windows fallback via `cryptography` when `openssl` absent; also probes Git for Windows `openssl.exe`

### Added

- **`scripts/p2p_tls_crypto.py`** вЂ” pure-Python CA/node cert generation
- **`tests/unit/test_p2p_tls_crypto.py`**
- **`prod_mesh_resilience_suite.ps1`** вЂ” preflight hint when mesh unreachable (e.g. placeholder `RPC_API_KEYS`)

### Changed

- **`docs/P2P_TLS.md`** вЂ” Windows TLS generation note

---

## [1.2.75] вЂ” 2026-07-14

### Added

- **`scripts/verify_p2p_tls_mesh.py`** вЂ” static cert + live `/p2p/security.tls` verify for prod mesh
- **`scripts/p2p_tls_preflight.py`** + **`prepare_p2p_tls_mesh.ps1`** вЂ” TLS material preflight
- **`scripts/p2p_tls_evidence_suite.ps1`** вЂ” gen/start/verify TLS mesh + optional failover drill
...
- **`prod_mesh_resilience_suite.ps1`** вЂ” `-P2pTls` runs TLS verify after mesh probe
- **`verify_prod_mesh_probe.py`** вЂ” records `p2p_tls_enabled` / `p2p_tls_ready` in deep probe
- **`docs/P2P_TLS.md`** вЂ” evidence suite, soak, monolith gate workflow

---

## [1.2.74] вЂ” 2026-07-14

### Added

- **`scripts/verify_prod_mesh_probe.py`** + **`probe_prod_mesh.ps1`** вЂ” structured prod mesh verify (`:18180-:18182`, chain 778888)
- **`scripts/prod_mesh_resilience_suite.ps1`** вЂ” probe + stabilize + failover + optional DR rehearsal (no soak)
- **`scripts/ceremony_evidence_suite.ps1`** + **`prepare_ceremony_deploy.ps1`** вЂ” genesis ceremony preflight path
...
## [1.2.73] вЂ” 2026-07-14

### Added

- **`scripts/bridge_cutover_evidence_suite.ps1`** вЂ” unified bridge L1 cutover path (`-RpcOnly` / `-Full` / `-Live`)
- **`scripts/prepare_bridge_l1_cutover.ps1`** вЂ” wrapper for cutover evidence suite
- **`.env.bridge.cutover.example`** вЂ” L1 RPC + contract env template for prod cutover
- **`scripts/testnet_backup_restore.ps1`** вЂ” Docker testnet seed backup + optional DR rehearsal
- **`scripts/testnet_log_rotate.sh`** вЂ” rotate `data/node.log` inside testnet containers (VPS cron)
- **`tests/unit/test_bridge_cutover_evidence.py`**

### Changed

- **`docs/BRIDGE_L1_MAINNET.md`** вЂ” evidence suite workflow
- **`docs/VPS_DEPLOY.md`**, **`docs/PUBLIC_TESTNET.md`** вЂ” backup/restore + log rotation checklist
- **`bridge_l1_cutover.py`** вЂ” hint for evidence suite on placeholder contract failures

---

## [1.2.72] вЂ” 2026-07-14

### Added

- **`scripts/testnet_dns_cutover.py`** вЂ” DNS resolve + HTTPS `/api` probe before public cutover
- **`scripts/prepare_testnet_dns_cutover.ps1`** вЂ” workstation wrapper for DNS/TLS verification
- **`scripts/vps_testnet_bootstrap_mesh3.sh`** вЂ” Linux VPS 3-node testnet mesh bootstrap
- **`vps_testnet_preflight.py --mesh3` / `--domain`** вЂ” mesh3 deploy steps + optional HTTPS cutover probe
- **`tests/unit/test_testnet_dns_cutover.py`**

### Changed

- **`vps_testnet_bootstrap.sh`** вЂ” optional `--mesh3` / `MESH3=1` for validator overlay
- **`deploy/nginx/testnet.example.conf`** вЂ” port 80 ACME + HTTPS redirect for certbot
- **`prepare_vps_testnet.ps1`** вЂ” `-Mesh3`, `-Domain` flags
- **`docs/VPS_DEPLOY.md`**, **`docs/PUBLIC_TESTNET.md`** вЂ” VPS mesh3 + DNS cutover path

---

## [1.2.71] вЂ” 2026-07-14

### Added

- **3-node public testnet mesh** вЂ” `docker-compose.testnet.mesh3.yml`, `docker/node.testnet.validator3.json`, ports `:19082/:19088/:19502`
- **`scripts/docker_testnet_mesh3.ps1`** вЂ” start seed + 2 validators and verify sync
- **`scripts/testnet_health_watch.ps1`** вЂ” periodic mesh health polling (`-Mesh2` / `-Mesh3`)
- **`verify_testnet_mesh.py --mesh3`** вЂ” 3-node verify (`:19080/:19081/:19082`)
...
- **`.env.testnet.example`** вЂ” validator-3 port vars (`TESTNET_HTTP_PORT_3`, RPC, P2P)
- **`docs/PUBLIC_TESTNET.md`** вЂ” 3-node mesh + health watch checklist

---

## [1.2.70] вЂ” 2026-07-14

### Added

- **`scripts/verify_testnet_mesh.py`** вЂ” 2-node public testnet mesh verify (seed :19080 + validator :19081, `/testnet/mesh`)
- **`scripts/docker_testnet_mesh.ps1`** вЂ” start seed+validator and verify sync
- **`scripts/probe_testnet_mesh.ps1`** вЂ” quick port probe for testnet mesh
- **`public_testnet_gate.py --mesh`** вЂ” optional 2-node mesh check in live gate
- **`tests/unit/test_verify_testnet_mesh.py`**

### Changed

- **`docker/node.testnet.seed.json`** / **validator** вЂ” `testnet_expected_peers: 1` for 2-node mesh health
- **`testnet_evidence_suite.ps1`** вЂ” mesh verify when `-WithValidator`

---

## [1.2.69] вЂ” 2026-07-14

### Added

- **`scripts/testnet_uptime_probe.py`** + **`.ps1`** вЂ” cron-friendly testnet health snapshot (`logs/testnet_uptime.json`, optional `--append` jsonl)
- **`deploy/nginx/install_testnet_nginx.sh`** вЂ” VPS nginx site installer with domain substitution
- **`monolith_gate --vps-testnet-preflight`** / **`-VpsTestnetLive`**
- **`tests/unit/test_testnet_uptime_probe.py`**

### Changed

- **`testnet_evidence_suite.ps1`** вЂ” full path: seed в†’ public gate в†’ VPS preflight в†’ uptime probe
- **`testnet_readiness.ps1 -VpsPreflight`** вЂ” optional VPS preflight step
- **`docker-compose.testnet.yml`** вЂ” validator host ports `19081/19087/19501` (was `9081/9087/9501`)
- **`.env.testnet.example`** вЂ” validator port vars
- **`vps_testnet_bootstrap.sh`** вЂ” live preflight + uptime probe after seed boot
- **`docs/PUBLIC_TESTNET.md`**, **`docs/VPS_DEPLOY.md`**

---

## [1.2.68] вЂ” 2026-07-14

### Added

- **`--probe-l1-rpc-only`** вЂ” validate `ETH_RPC_URL` before L1 contracts are deployed (placeholder contracts в†’ WARN, not FAIL)
- **`scripts/vps_testnet_preflight.py`** + **`prepare_vps_testnet.ps1`** вЂ” VPS public testnet preflight (nginx template, env, public gate)
...
- **`docs/BRIDGE_L1_MAINNET.md`**, **`docs/PUBLIC_TESTNET.md`** вЂ” rpc-only vs full probe workflow

---

## [1.2.67] вЂ” 2026-07-14

### Added

- **`scripts/bridge_l1_live_probe.py`** + **`bridge_l1_live_probe.ps1`** вЂ” unified L1 bridge probe (`static` / `--probe-l1` / `--live` / `--full`); writes `logs/bridge_l1_live_probe.json`
...
- **`bridge_l1_cutover.py`** вЂ” includes `l1_rpc` probe summary in gate meta when `--probe-l1`
- **`mainnet_readiness.py`** вЂ” `--probe-l1` works without `--live` (fixes prior `probe_l1=live` coupling)
- **`docs/BRIDGE_L1_MAINNET.md`** вЂ” live probe workflow

---

## [1.2.66] вЂ” 2026-07-13

### Added

- **`scripts/gen_p2p_mesh_tls.py`** вЂ” generate CA + node1/node2/node3 P2P TLS material for prod Docker mesh
- **`docker-compose.prod.3node.p2ptls.yml`** вЂ” compose overlay with `/app/p2p_tls` mounts and `P2P_TLS_*` env
- **`docker_prod_3node.ps1 -P2pTls`** вЂ” auto-generate certs, start mesh with TLS overlay, verify `/p2p/security.tls.ready`
- **`tests/unit/test_gen_p2p_mesh_tls.py`**

### Changed

- **`docs/P2P_TLS.md`** вЂ” Docker prod mesh TLS section

---

## [1.2.65] вЂ” 2026-07-13

### Added

- **`scripts/soak_preflight.py`** вЂ” prod mesh readiness for 48h soak (health, P2P security, harness, topology); writes `logs/soak_preflight.json`; does **not** start soak
- **`scripts/prepare_48h_soak.ps1`** вЂ” PowerShell wrapper for preflight
- **`monolith_gate.py --soak-preflight`** and **`monolith_gate.ps1 -SoakPreflight`**
- **`tests/unit/test_soak_preflight.py`**

### Changed

- **`restart_soak_prod_mesh.ps1`** вЂ” dynamic `git describe` tag in evidence + soak metadata; preflight hint in output
- **`verify_prod_stack.py --live-prod-mesh`** вЂ” P2P security policy check on all three nodes

---

## [1.2.64] вЂ” 2026-07-13

### Added

- **Optional P2P wire TLS** вЂ” `network/p2p_tls.py`, config/env `P2P_TLS_*`, fail-closed start when enabled but misconfigured
- **`scripts/gen_p2p_dev_tls.py`** вЂ” dev CA + node cert generator (OpenSSL)
- **`docs/P2P_TLS.md`** вЂ” P2P TLS vs nginx HTTP TLS
- **`GET /p2p/security.tls`** вЂ” readiness block
...
## [1.2.63] вЂ” 2026-07-13

### Added

- **`prod-mesh3-ci-recovery`** вЂ” isolated ceremony spawn on `:15280вЂ“15282` + node2 SIGTERM/rejoin drill (GitHub Actions Linux)
- **`verify_spawn_mesh3_recovery()`** вЂ” process-based failover for CI (mirrors Docker `prod-mesh3-recovery`)
- **`verify_p2p_ci.py --recovery`** вЂ” append failover drill to `--mode prod-mesh3`
...
## [1.2.62] вЂ” 2026-07-13

### Added

- **Rate-limit exempt wire types** вЂ” `block`, `blocks`, `status`, handshake/ping/state-root not counted (safe sync bursts on prod hub)
- **`industrial_gate` P2P hardening check** вЂ” static allowlist, security surface, config defaults
...
## [1.2.61] вЂ” 2026-07-13

### Added

- **Handshake chain_id mismatch в†’ strike/ban** вЂ” wrong-network peers accumulate strikes; `handshake_rejects` in `/p2p/security`
- **`docker_prod_3node.ps1 -RecoveryDrill`** вЂ” runs `prod-mesh3-recovery` after mesh boot
- **Recovery drill** now validates P2P security on all nodes after node2 rejoin

### Fixed

- **Rate limit no longer bans peers** вЂ” excess messages are dropped only (sync bursts were false-banning prod hub)
- **Wire EOF/parse close** вЂ” disconnect without strike/ban on peer close (fixes prod mesh split)
...
## [1.2.60] вЂ” 2026-07-13

### Fixed

- **`probe_mesh_nodes.ps1`** вЂ” PowerShell parse error from UTF-8 em dash; `-Deep` also tries `/p2p/security` when topology lacks `security`
- **`verify_p2p_security_mesh`** вЂ” fallback to `/p2p/topology.security` on 404; WARN (not FAIL) when `status.p2p_summary` missing on older nodes; rebuild hint for prod mesh

---

## [1.2.59] вЂ” 2026-07-13

### Added

- **P2P `_maintenance_loop`** вЂ” periodic stale/unhealthy peer eviction and ban expiry
- **`GET /status.monolith_summary`** вЂ” compact readiness snapshot (P2P, consensus, native crypto, bridge)

---

## [1.2.58] вЂ” 2026-07-13

### Added

- **`GET /status.p2p_summary`** вЂ” compact mesh health + security snapshot (peer scores, bans, rate limits)
- **`verify_p2p_security_mesh()`** in `verify_p2p_ci.py` вЂ” validates `/p2p/security` and status summary on all mesh nodes
...
- **`GET /p2p/security`** вЂ” rate limits, active bans, eviction policy (also embedded in `/p2p/topology.security`)
...
## [1.2.56] вЂ” 2026-07-13

### Added

- **`scripts/monolith_gate.py`** вЂ” unified static gate: industrial + mainnet readiness + launch checklist в†’ `data/monolith_gate.json`
- **`scripts/monolith_gate.ps1`** вЂ” PowerShell wrapper
...
## [1.2.55] вЂ” 2026-07-13

### Fixed

- **P2P auto mode:** `-P2P` / `--prefer-devnet` no longer hijacks live prod mesh on `:18180вЂ“:18182`; use `--prefer-prod-mesh` or `-ProdMesh` for prod checks
- **Harness timeouts:** `verify_p2p_ci` uses `_consistency_harness()` with `quick=1` and в‰Ґ45s HTTP timeout on prod ports (fixes false `node1 harness: timed out`)

---

## [1.2.54] вЂ” 2026-07-13

### Added

- **`-ProdMeshFull`** on `test_blockchain_full.ps1` вЂ” after `-ProdMesh` gates runs `prod_evidence_suite` (stabilize, health, failover drill, signed tx, EVM smoke)
- **`scripts/prod_mesh_full.ps1`** вЂ” one-command alias for full prod mesh ops proof
- **`prod_evidence_suite.ps1`** вЂ” `-FailoverWaitSec` parameter (passed to `prod_mesh_failover.ps1`)
...
## [1.2.53] вЂ” 2026-07-13

### Added

- **`-ProdMesh` / `-ProdMeshSpawn`** on `test_blockchain_full.ps1` вЂ” live prod 3-node P2P gate (`prod-mesh3-live`) + deep mesh probe + `mainnet_readiness --live-prod-mesh`
- **P2P industrial hardening:** max wire message size (`p2p_max_message_bytes`), peer health scores in topology/`/p2p/peer-score`
- **`verify_p2p_ci --mode auto`** now detects prod mesh on `:18180вЂ“:18182` before devnet
- **`probe_mesh_nodes.ps1 -Deep`** вЂ” topology + consistency harness summary (auto on `-ProdMesh`)

### Tests

- `tests/unit/test_p2p_industrial.py` вЂ” oversized message drop, auto prod-mesh detection

---

## [1.2.52] вЂ” 2026-07-13
...
## [1.2.41] вЂ” 2026-07-13

### Added

- `scripts/mesh_heal_fork.ps1` вЂ” reseed node1 chainstore from node2 when hub diverges
...
## [1.2.27] вЂ” 2026-07-12

### Added

- `RELEASE_NOTES_v1.2.27.md` вЂ” verification mermaid flows + copy-paste prod mesh checks

### Fixed

- **Prod mesh mining stall** вЂ” `mesh_ready_for_mining` no longer latches on stale P2P wire roots; STATUS height alignment fallback
- **Event loop freeze** вЂ” hub P2P broadcast non-blocking; `blockchain.add_block` via `asyncio.to_thread` (EVM deploy no longer blocks mining)
- **Parallel state-root RPC** вЂ” faster peer queries; sync engine skips mismatch while peer catching up
- **EVM deploy txs** вЂ” `tx_validator` allows zero-value deploy; `prod_evm_smoke.py` mempool-only cross-node path (no direct deploy fallback)
- **Prod JWT** вЂ” `verify_p2p_ci._mint_admin_jwt_from_secret()` for mesh when `/auth/token` disabled
...
- `tests/unit/test_mesh_mining_ready.py` вЂ” stale wire + STATUS height fallback cases

---

## [1.2.26] вЂ” 2026-07-06

### Added

- `scripts/prod_evm_smoke.py` вЂ” prod HTTP deploy + `eth_getStorageAt` on RPC :18546вЂ“:18548
- `scripts/prod_evidence_suite.ps1` вЂ” one-shot health + failover + signed tx + EVM

### Fixed

- `prod_signed_tx_smoke.py` вЂ” missing `import time`

---

## [1.2.25] вЂ” 2026-07-06

### Docs

- `docs/EVIDENCE_MATRIX.md` вЂ” proven vs not-proven ops (failover, signed tx, EVM prod, soak 24h+, audit)
- README / MAINNET_GAP / PUBLIC_TESTNET aligned with live prod mesh evidence gaps

---

## [1.2.24] вЂ” 2026-07-06
...
## [1.2.23] вЂ” 2026-07-06

### Added

- Rocks read path for tx propagation (`get_tx_propagation_trace`, `get_recent_tx_propagation`)
- `scripts/testnet_readiness.ps1` вЂ” automated PUBLIC_TESTNET local prerequisites

---

## [1.2.22] вЂ” 2026-07-06

### Added

- `RocksEngine.state_root_from_account_prefix` вЂ” native Rocks scan + state root without Python blob round-trip
...
- `soak_monitor.ps1` вЂ” hashtable splat to `health_watch` (long soak no longer exits in 50ms); stricter pass criteria

---

## [1.2.20] вЂ” 2026-07-06
...
## [1.2.17] вЂ” 2026-07-06

### Added

- `scripts/soak_monitor.ps1` вЂ” 24h+ prod mesh soak with JSON report
- Harness `?quick=1&peer_timeout=3` for fast monitoring polls

### Fixed

- `health_watch.ps1` вЂ” harness timeout, quick/full cycle, mesh height alignment, `failed_checks` array handling
...
- `scripts/health_watch.ps1` вЂ” prod mesh poll + optional webhook

### Changed

- `MAINNET_GAP_ANALYSIS.md`, `INCIDENT_RESPONSE.md`, `REPO_PROFILE.md` вЂ” RocksDB DR, 703 tests, CI refs
- `OBSERVABILITY.md`, README вЂ” health watch docs

---

## [1.2.14] вЂ” 2026-07-05

### Added

- `docs/ARCHITECTURE.md` вЂ” honest mermaid architecture (prod vs dev paths)
- `docs/PUBLIC_TESTNET.md` вЂ” public testnet go-live checklist (not live)
- `.github/workflows/security-audit.yml` вЂ” pip-audit on `requirements.txt`
...
- `Dockerfile.prod` вЂ” dummy `src/lib.rs` before `cargo fetch` for abs_native layer cache
...
- `dr_restore_rehearsal.ps1` вЂ” explicit `-DockerMesh1` call (array splat broke switch binding on PS 5.1)
...
- `RocksEngine` вЂ” `read_only=True` opens DB with `open_for_read_only` for live backups
...
- `backup_chainstore.ps1 -DockerMesh1` вЂ” stdin-piped checkpoint backup (no `/app/scripts/` in old prod images)

### Added

- `scripts/docker_backup_in_container.py` вЂ” minimal in-container backup via `docker exec python -`
...
- `scripts/dr_restore_rehearsal.ps1` вЂ” ASCII-only strings (Windows PowerShell 5.1 parse error on em-dash)

### Added

- `scripts/bench_storage_commit.py` вЂ” SQLite vs RocksDB commit latency benchmark
...
- RocksDB tuning: `ROCKSDB_BLOCK_CACHE_MB`, `ROCKSDB_WRITE_BUFFER_MB` в†’ native `RocksEngine`
- LSM property introspection in `get_stats()` (`rocksdb_properties`)
- `scripts/dr_restore_rehearsal.ps1` вЂ” temp restore verify without touching live data
- `RELEASE_NOTES_v1.2.3.md`

### Changed

- `docs/STORAGE_ROCKSDB.md` вЂ” aux.db permanent scope + tuning table
...
- `docs/STORAGE_ROCKSDB.md` вЂ” honest hybrid RocksDB architecture + roadmap
- `storage/chain_backup.py` вЂ” backup/restore for Rocks chainstore and SQLite
- `scripts/backup_chainstore.py`, `restore_chainstore.py`, `backup_chainstore.ps1`
- `scripts/backup_rocks_drill.py` вЂ” CI DR drill for RocksDB
...
- `.github/workflows/docker-prod-image.yml` вЂ” BuildKit + GHA cache; publishes `ghcr.io/gruver87/abs-blockchain-node` on master/tags
- `-PullLatest` / `--pull-latest` for prod mesh scripts (uses GHCR image via `ABS_PROD_IMAGE`)
- `docs/DOCKER_IMAGES.md` вЂ” honest Docker/GHCR ops guide
- BuildKit cache layers in `Dockerfile.devnet-rust` (same pattern as prod)

### Changed

- `docker-compose.prod.3node.yml` вЂ” `ABS_PROD_IMAGE` override for prebuilt images
- Prod mesh README section вЂ” GHCR pull path documented
...
- `GET /status` вЂ” `p2p_sync_status`, `peers_connected`, `validators_registered`, `mesh_min_peers`, `bridge_disabled_reason`
- `GET /bridge/status` вЂ” alias for bridge overview
- `scripts/probe_mesh_nodes.ps1` вЂ” multi-port mesh/bridge/features probe
- `LightClient.sync_headers_from_peers()` for untrusted peer headers
- `tests/unit/test_light_client_sync.py`

### Fixed

- Light client local bootstrap: `sync_from_blockchain()` uses trusted sequential `add_header()` (was failing peer validation on local DB в†’ вЂњ0 headers syncedвЂќ)
- Explorer dashboard: contextual P2P badges; deployment mode row; bridge off reason
- `scripts/full_audit.py`: solo/stale/under-mesh warnings instead of generic вЂњinconsistentвЂќ
...
## [1.2.0-industrial] вЂ” Wave 37вЂ“63 (РёСЋРЅСЊ 2026)

### Wave 63 вЂ” Admin lockdown for node repair endpoints
...
- **`api_wave` remains 61** вЂ” Wave 63 hardens access policy, not the REST API surface

### Wave 62 вЂ” Live Docker recovery gate

- `verify_p2p_ci.py --mode devnet3-recovery` вЂ” live 3-node Docker recovery drill that stops `node2`, keeps `node1/node3` consistent, restarts `node2`, and requires mesh rejoin plus matching `state_root`
- `scripts/docker_devnet_3node.ps1 -Recovery` вЂ” optional industrial gate after normal 3-node verification
- Recovery assertions verify persistent heights, root convergence, peer rejoin, and `topology_healthy=true` after restart
- **`api_wave` remains 61** вЂ” Wave 62 hardens live verification, not the REST API surface

### Wave 61 вЂ” Network hygiene + real peer rejoin
...
- `GET /p2p/topology` / `GET /p2p/peer-score` вЂ” live peer graph, known rejoin candidates, height gaps, last-seen ages, and topology health
- `POST /p2p/reconnect` вЂ” actively reconnect bootstrap/known peers from the unified node runtime
- `scripts/docker_devnet_3node.ps1` вЂ” host-port guard prevents local `python main.py` from being mixed with Docker devnet ports
- **`api_wave` в†’ 61**

### Wave 60 вЂ” CI L1 RPC + relayer live e2e

- `bridge/mock_l1_rpc.py` вЂ” in-process Ethereum JSON-RPC endpoint for isolated CI
- `GET /testnet/bridge-relayer-proof` вЂ” relayer readiness dashboard
- `verify_p2p_ci.py` вЂ” `verify_bridge_relayer()` + `--mode ci-bridge-relayer`
- **`api_wave` в†’ 60**

### Wave 59 вЂ” Bridge relayer e2e + Explorer fork UI

- `RustBridge.enqueue_l1_incoming()` вЂ” L1 incoming queue for relayer watch
- `POST /bridge2/transfer` вЂ” routes through `RustBridge` when enabled (incoming/outbound)
- `POST /bridge/oracle/l1-register` вЂ” enqueues incoming/outbound L1 queue entries
- Explorer вЂ” Testnet Fork Monitor card, `l1_tx_hash` on bridge forms, `bridge2` RustBridge path
- `verify_p2p_ci.py` вЂ” `verify_bridge()` after adversarial; `--mode ci-bridge` isolated test
- `tests/unit/test_bridge_relayer_e2e.py` вЂ” lock в†’ queue в†’ relayer incoming e2e
- **`api_wave` в†’ 59**

### Wave 58 вЂ” Fork CI (partition + recovery)

- `GET/POST /testnet/fork-exercise` вЂ” fork-status before/after + P2P reconcile drill
- `verify_p2p_ci.py` вЂ” `verify_fork_recovery()` after multi-node proof
- `--mode ci-fork` вЂ” real partition test: stop follower node, mine ahead, restart, reconcile
- **`api_wave` в†’ 58**

### Wave 57 вЂ” Real core (no random stubs in consensus path)

- **Deterministic proposer** вЂ” `ConsensusEngine` + `ValidatorSelection.select_proposer_weighted`; removed `random` fallbacks and AI-validator mining shortcut
- **Finality quorum** вЂ” `FinalityEngine` uses live validator count (not hardcoded 32)
- **Reorg finality guard** вЂ” `Blockchain.reorg_to_ancestor()` refuses rollback below finalized checkpoint
- **P2P reorg** вЂ” `ReorgPredictor.analyze_live_peers()` wired into fork reconcile
- **MEV** вЂ” fee-ordering analysis from mempool (no `random.uniform` profits)
- **Bridge honesty** вЂ” Python bridge adapter only for explicit dev/test paths; Docker uses `RustBridge`
- `GET /status` в†’ `core_real` flags; **`api_wave` в†’ 57**

### Wave 56 вЂ” Multi-node proof (3-validator devnet)

- `docker/validators.devnet3.json` вЂ” 3 miners + attesters; `node*.devnet3.rust.json` configs
- `GET /testnet/multi-node-proof` вЂ” mesh + harness + validators + attestations + `proof_ok`
- `POST /testnet/reorg-exercise` вЂ” canonical replay drill (`reorg_safe` flag)
- Proposer rotation threshold: `distinct_proposers >= 3` when `expected_validators >= 3` and height в‰Ґ 12
- `verify_p2p_ci.py` вЂ” `verify_multi_node_proof()` after state harness (attestations, rotation, reorg drill)
- **`api_wave` в†’ 56**

### Wave 55 вЂ” 5-validator devnet

- `docker-compose.devnet-5validator.yml` вЂ” 5 nodes `:8080`вЂ“`:8084`, 3 miners + 2 attesters
- `docker/validators.devnet5.json` вЂ” manifest; addresses derived at runtime (no keys on disk)
- `GET /testnet/validators` вЂ” validator set health, proposer rotation stats
- Mining proposer gate вЂ” only selected validator forges when `active_validators > 1`
- `verify_p2p_ci.py --mode devnet5`; `.\scripts\docker_devnet_5validator.ps1`
- Devnet5 sync fix вЂ” seeded-chain `dev_signer` skip, `ensure_state_at_tip` replay at tip
- **`scripts/full_audit.py`** вЂ” unified audit: syntax, Waves 52вЂ“55, secrets, mega/final, pytest, live API, P2P
- `verify_p2p_ci.py` вЂ” unique tx recipient per run (no false fail on repeat audit)

### Wave 54 вЂ” State consistency harness

- `GET /chain/consistency/harness` вЂ” tip alignment, peer roots, supply cap, mismatch audit
- `GET /testnet/state-consistency` вЂ” alias for harness on multi-node devnet
- `POST /chain/consistency/repair` вЂ” replay chain when live state drifted from tip
- `verify_p2p_ci.py` вЂ” cross-node harness check + auto-repair in devnet/ci3 modes

### Wave 53 вЂ” Fork / slashing / partition CI

- `GET /testnet/fork-status` вЂ” divergent heads, height gaps, `consensus_healthy`, slash summary
- `GET /slashing/events` вЂ” persisted slash events from SQLite
- `verify_p2p_ci.py --mode ci3` / `ci-adversarial` вЂ” isolated 3-node + double-vote slash test
- Atomic `reorg_to_ancestor` rollback; `ensure_state_at_tip()` on boot; staking catch-up only on miner

### Wave 52 вЂ” 3-node testnet (Docker)

- `docker-compose.devnet-3node.yml` вЂ” node1 `:8080`, node2 `:8081`, node3 `:8082`
- `GET /testnet/mesh` вЂ” peer heights, `mesh_healthy`, `expected_peers`
- `verify_p2p_ci.py --mode devnet3` вЂ” 3-node sync + tx on node2 **and** node3 mempools
- `.\scripts\docker_devnet_3node.ps1` вЂ” seed DB, force-recreate, CI verify
- Faucet top-up in verify when dev signer balance low

### Wave 51 вЂ” Transaction propagation (P2P)

- Full signed tx gossip + mempool pull sync (`get_mempool` / `mempool` P2P messages)
- SQLite `tx_propagation_events` вЂ” lifecycle: submit в†’ mempool в†’ P2P в†’ block в†’ receipt
...
### Wave 50 вЂ” Strict state_root on all nodes

- `state_root_strict_p2p` (default `true`) вЂ” P2P import rejects `state_root` mismatch above baseline
- `GET /chain/state-root/status` вЂ” local root, peer comparison, policy, recent mismatches
- SQLite `state_root_mismatches` audit log; pruned on reorg
- `/sync/status` includes `state_root_strict_p2p` and policy fields

### Wave 49 вЂ” Block proposer audit log

- `block_proposer_audit` SQLite table on every confirmed block
- Backfill from historical `blocks` on node start
- `GET /chain/proposers/stats` вЂ” top proposers by block count
- `GET /chain/proposers/history` вЂ” paginated audit log (`proposer` filter)
- `GET /chain/proposer/{addr}` вЂ” proposer detail + recent blocks
- Pruned on reorg; `proposer_audit_count` in `/chain/metrics`

### Wave 48 вЂ” Address tx index + receipt backfill

- `GET /address/{addr}/activity` вЂ” balance, sent/received counts, last tx height
- `GET /address/{addr}/txs` вЂ” paginated history (`limit`, `offset`, `direction=sent|received|all`)
- Idempotent backfill: historical `transactions` в†’ `tx_receipts` on each node start

### Wave 47 вЂ” Core L1 receipts + chain metrics

- `tx_receipts` SQLite table on every confirmed tx
- `GET /chain/metrics` вЂ” avg block time, tx/receipt counts
- `GET /tx/receipt/{hash}`, `GET /receipts/block/{height}`
- Receipts pruned on reorg (`truncate_chain_state`)

### Wave 46 вЂ” NFT SQLite persistence

- NFT tokens, offers, auctions, sales history РІ SQLite
- Genesis collection seed РїСЂРё РїСѓСЃС‚РѕР№ Р‘Р”; mint/buy/transfer СЃРѕС…СЂР°РЅСЏСЋС‚СЃСЏ
- `GET /nft/stats`, `nft_persisted` РІ `/l2/status`

### Wave 45 вЂ” Reorg predictor + dev bridge

- SQLite-РёСЃС‚РѕСЂРёСЏ РѕС†РµРЅРѕРє СЂРµРѕСЂРіР° (`reorg_assessments`)
- РСЃРїСЂР°РІР»РµРЅС‹ `GET /reorg/depth`, `/reorg/fork`, РґРѕР±Р°РІР»РµРЅС‹ `/reorg/history`
- `GET /features` вЂ” `api_wave`, `l2_modules`, РїРѕРґСЃРєР°Р·РєР° `bridge_dev_confirm`
- Dev: `POST /bridge/confirm-pending` Рё alias `/bridge/dev-confirm-pending` (Р±РµР· HMAC)

### Wave 44 вЂ” L2 dashboard + MEV history

- `GET /l2/status` вЂ” РµРґРёРЅС‹Р№ РґР°С€Р±РѕСЂРґ Lightning / Plasma / Will / WASM / AI
- MEV analyzer: РёСЃС‚РѕСЂРёСЏ РІ SQLite, `GET /mev/history`

### Wave 43 вЂ” AI agents

- AI agents / trades РІ SQLite, create fee 0.01 ABS
- Plasma `submit-block`: РїРѕРґСЃРєР°Р·РєРё РїСЂРё РїСѓСЃС‚РѕР№ РѕС‡РµСЂРµРґРё

### Wave 42 вЂ” WASM + relayer status

- WASM VM: РєРѕРЅС‚СЂР°РєС‚С‹ / storage / events РІ SQLite, deploy fee 0.01 ABS
- `GET /bridge/relayer/status` вЂ” L1 queue + pending locks

### Wave 41 вЂ” Crypto Will

- Р—Р°РІРµС‰Р°РЅРёСЏ РІ SQLite: create Р±Р»РѕРєРёСЂСѓРµС‚ L1, execute в†’ heir, cancel в†’ refund
- `POST /will/execute` (`force=true` РІ dev)

### Wave 40 вЂ” L2 persistence

- Lightning: РєР°РЅР°Р»С‹ РІ SQLite, open/close РІР»РёСЏРµС‚ РЅР° L1 ABS
- Plasma: deposits / blocks / exits РІ SQLite, deposit/exit РІР»РёСЏРµС‚ РЅР° L1

### Wave 39 вЂ” Oracle registry + bridge L1 queue

- HMAC-signed oracle feeds РІ SQLite (`GET /oracles/feeds`, `POST /oracles/feeds/submit`)
- `POST /bridge/lock` СЃ `l1_tx_hash` в†’ `data/bridge_l1_queue.json`
- `GET /bridge/l1-queue`, alias `GET /oracles/l1-queue`

### Wave 37вЂ“38 вЂ” EVM hardening + P2P

- EVM: LOG, EXTCODE, SELFDESTRUCT, BLOCKHASH, CALLCODE; bytecode validator РІ mempool
- EVM logs РІ SQLite (`GET /evm/logs`)
- Sharding: cross-shard СЂРµР°Р»СЊРЅС‹Рµ РїРµСЂРµРІРѕРґС‹ Р±Р°Р»Р°РЅСЃРѕРІ
- Bridge: `l1_tx_hash` РѕР±СЏР·Р°С‚РµР»РµРЅ РїСЂРё `ETH_RPC_URL`
- РЎРµРєСЂРµС‚С‹ С‚РѕР»СЊРєРѕ РІ `.env`, С‡РµСЃС‚РЅР°СЏ РґРѕРєСѓРјРµРЅС‚Р°С†РёСЏ РІ `docs/ALL_COMMANDS.txt`

---

## РџСЂРѕРІРµСЂРµРЅРѕ Р»РѕРєР°Р»СЊРЅРѕ
...
## Р§РµСЃС‚РЅРѕ: С‡С‚Рѕ СЌС‚Рѕ **РЅРµ** РґР°С‘С‚

- РќРµ production mainnet
- РќРµ РїРѕР»РЅС‹Р№ EVM / РЅРµ Ethereum-СЃРѕРІРјРµСЃС‚РёРјРѕСЃС‚СЊ РЅР° 100%
- Bridge / Lightning / Plasma / MEV вЂ” dev/test or analysis modules with real L1 effects where stated
- РљСЂРёРїС‚Рѕ-Р°СѓРґРёС‚ РЅРµ РїСЂРѕРІРѕРґРёР»СЃСЏ

РЎРј. [DISCLAIMER.md](DISCLAIMER.md) Рё **Р§Р°СЃС‚СЊ 0** РІ [docs/ALL_COMMANDS.txt](docs/ALL_COMMANDS.txt).
