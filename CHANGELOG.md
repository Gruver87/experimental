...
- **LR autonomous WS + hard-audit heal (2026-09-26):** after tip-plateau fix, deep ADR 0017 audit found adopt-above-tip brick, O(gap) ancestry rebuild per import, and committee `OSError` peer tears. Landed: `consensus/long_range/roll_forward.py` (miner roll at tip−confirm, gap 512); gossip `ahead_of_tip` / `equivocation`; tip-safety light `_advance_after_import`; WS rate-limit; staging LR hard-off; wire-roots 3s cache; under-mesh reconnect ≥8s. Docs: STRICT_SOAK_PARITY / LONG_RANGE_LAB_PROFILE / EXECUTION_ORDER / ADR 0017 addendum. Unit+gate green. **Soak not re-started** — next operator step: STRICT LR 48h on command only.
- **LR STRICT tip-plateau root fix (2026-09-26):** mid-soak stopped at ~24h (fail_lines=2). Cascade: sticky state_consistent=False + mesh_min=2 hard-skip mining → tip plateaus → GIL HOL → ready dual-timeout. Fixes: under-mesh reconnect in mining; wire_soft_fail STATUS-unanimous forge (lab only); no force_inconsistent on probe exception outside prod; wire sticky empty max=5; health_watch sibling re-probe. Soak not re-started.
...
- **Libp2p STRICT 48h PASS (2026-09-23→25):** pack [`docs/evidence/runs/lp2pstrict1/`](docs/evidence/runs/lp2pstrict1/) — `passed=true` strict=true hard_fails=0 mesh_warn=0 warn_lines=0 fail_lines=0 mesh_ok=2800 tip ~57209→~68082 IntervalSec=60 FullHarnessEvery=6. Soft tip_skew/transient_delta OK-only. Harden: peer_probe 3×, under_mesh reconnect, soft-only harness never Strict FAIL; scripts `start_soak_prod_mesh_48h_strict.ps1` / `start_soak_evm_mesh_48h_strict.ps1` / LR `-Strict`. Triage: [`docs/sprouts/STRICT_SOAK_PARITY.md`](docs/sprouts/STRICT_SOAK_PARITY.md). **Not** mempool sidecar / not default ind48pass1 / not mainnet / not BLS.
- **Industrial tip 48h PASS + ADR 0021 mesh proof (2026-09-21→23):** Experimental prod mesh soak on git `719deb4` / image `23a4a8fa` — pack [`docs/evidence/runs/ind48pass1/`](docs/evidence/runs/ind48pass1/) (`passed=true`, `hard_fails=0`, `mesh_warn=0`, `warn_lines=0`, tip ~46099→~56972, 575 mesh-ok cycles). Proves ADR 0021 wire satoshi + `p2p_mempool_require_wire_satoshi` under live mesh load after industrial HIGH/MED/CI heal. ADR honesty + EVIDENCE_MATRIX / EXECUTION_ORDER / FUND_READINESS updated. **Not** STRICT mempool sidecar (see [`mempool48pass1`](docs/evidence/runs/mempool48pass1/)). **Not** mainnet / not Hybrid / not BLS.
...
- **48h mempool+validation STRICT dual-report PASS (2026-09-17→19):** evidence [`mempool48pass1`](docs/evidence/runs/mempool48pass1/) — soak `passed=true` hard_fails=0 mesh_warn=0 ready_only=0 (2589 mesh_ok, warn_lines=31 soft, FullHarnessEvery=6, tip ~35187→~44020) + sidecar `passed=true` admit_ok=303 admit_soft=164 refuse_ok=467 refuse_fail=0 admit_fail=0, store=rust demoted=False. Prior FAIL 2026-09-13→15 (35) and 6h dual PASS 2026-09-16 remain on record. **Not** mainnet / **not** Hybrid / **not** Long-Range / **not** BLS.
- **6h mempool+validation STRICT PASS (2026-09-16):** dual-report green — soak `passed=true` hard_fails=0 mesh_warn=0 (351 cycles, tip 30650→32008) + sidecar `passed=true` admit/refuse 114/114 fail=0, store=rust demoted=False. AlwaysFullHarness@60s. **Not** 48h / **not** mainnet / **not** Hybrid. Residual: 95× soft WARN `p2p=inconsistent` were tip-race labels (mesh always aligned) — fixed via `tip_skew`/`tip_lagging` split.
- **p2p_sync_status tip honesty (2026-09-16):** `_derive_p2p_sync_status` reserves `inconsistent` for `state_consistent=False`; peer_gap≤2 + consistent → `tip_skew`, gap 3–20 → `tip_lagging`. health_watch no longer WARN on `tip_skew` (6h soak noise). Unit: `test_derive_p2p_sync_status_helpers`.
...
- **48h mempool+validation STRICT FINISHED_FAIL triage (2026-09-15):** 35 FAIL = 14× tip delta=1 (confirm too short under load) + 17× ready_flap 503 (Strict hard-FAIL while `/status` live) + 4× harness_timeout (Aligned forced false). Fixes: multi-pass Strict confirm (4×3s); ready_flap soft when status OK; harness_timeout keeps tip aligned; long Strict (`Hours>=12`) uses FullHarnessEvery=6 not AlwaysFullHarness; sidecar timeouts soft + backoff, default interval 300s. Prior run: tip 22114→30613, 445× strict_confirm, sidecar admit 724/1219 — **not** a 48h PASS claim. Soak not re-started.
- **Mempool miner pack + validation soak prep (2026-09-13):** `Mempool.get_for_block` (nonce-contiguous) wired into live miner/`PBS`/`MEV` via `abs_to_wei` (no `fee*1e9`). Harness uses shared pack. Sidecar `mempool_validation_sidecar.py` + `start_mempool_validation_soak.ps1` ready — soak **not** started. Not 48h / not wire float cutover.
