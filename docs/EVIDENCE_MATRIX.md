...
| Host verify restore (`verify_industrial_waves` + full `pytest tests/`) | **PASS** 2026-09-21 | Waves 542 needles OK; pytest **2734 passed / 11 skipped** after native-registry demote restore + honesty/needle drift. Covers MED wire-satoshi refuse + monitor HOL units already on `main`. **Not** mesh probe / **not** 48h soak. |
| Prod mesh probe after restore tip (`probe_prod_mesh -Quick`) | **PASS** 2026-09-21 | `logs/prod_mesh_probe.json` — tip ~44442 aligned 3/3; `verify_pre_soak -SkipPrepare` PASS. **Not** 48h soak. |
