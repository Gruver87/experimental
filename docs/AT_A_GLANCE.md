...
| 48h soak here | TCP+TLS **PASS** (2026-08-20→22, `0a7932c4`) — not libp2p. libp2p industrial soak **not run** until a new 48h after ADR 0020 |
...
|--------------|-------------|
| rust-libp2p swarm A–DB behind Cargo `libp2p` | libp2p 48h soak PASS (3-node mesh proven; soak not run) |
