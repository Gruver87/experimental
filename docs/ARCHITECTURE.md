...
flowchart TB
  ...
  subgraph net ["Network plane"]
    ...
    LP["rust-libp2p · ADR 0019 A–CZ · FEATURE_LIBP2P opt-in"]
  end
...
| [0019](adr/0019-rust-libp2p-industrial.md) | rust-libp2p | Slices **A–CZ** (phase 103) behind Cargo `libp2p`; advertised unique cap 20 |
...
| Lab swarm | rust-libp2p ADR 0019 **A–CZ** (phase 103) | Cargo `libp2p` + `FEATURE_LIBP2P` |
| Advertise | unique cap **20**; circuit never in crate book | Identify/mDNS/Kad/AutoNAT/UPnP/DCUtR omit uncharged; relay-client circuit confirm omitted; AutoNAT/UPnP confirm gated; observed confirm charges canonical key |
