...
flowchart TB
  ...
  subgraph net ["Network plane"]
    ...
    LP["rust-libp2p · ADR 0019 A–CV · FEATURE_LIBP2P opt-in"]
  end
...
| [0019](adr/0019-rust-libp2p-industrial.md) | rust-libp2p | Slices **A–CV** (phase 99) behind Cargo `libp2p`; advertised unique cap 20 |
...
| Lab swarm | rust-libp2p ADR 0019 **A–CV** (phase 99) | Cargo `libp2p` + `FEATURE_LIBP2P` |
| Advertise | unique cap **20**; circuit `/p2p-circuit` outside cap | Identify/mDNS/Kad/AutoNAT/UPnP/DCUtR omit uncharged |
