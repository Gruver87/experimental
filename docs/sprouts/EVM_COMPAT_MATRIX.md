...
| `eth_getTransactionReceipt` | **Partial** | Core fields + logs + **logsBloom from address/topics**. `cumulativeGasUsed` is the running sum of `gas_used` in block tx order (QueryFacade `get_block`); without a block listing it equals this receipt's `gasUsed`. `blockHash` / `blockNumber` come from the tx row or block listing — **never** the 32-byte zero digest or height `0` for a missing inclusion (`null` if unobserved). `burned` is integer satoshi (wire `burned` is ABS display × 1e6) |
