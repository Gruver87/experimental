...
| `eth_getTransactionReceipt` | **Partial** | Core fields + logs + **logsBloom from address/topics**. `cumulativeGasUsed` is the running sum of `gas_used` in block tx order (QueryFacade `get_block`); without a block listing it equals this receipt's `gasUsed`. `blockHash` comes from the tx row or block listing — **never** the 32-byte zero digest (`null` if unobserved) |
