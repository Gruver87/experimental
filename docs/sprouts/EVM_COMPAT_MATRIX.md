...
| `eth_newFilter` / `eth_getFilterChanges` / `eth_uninstallFilter` | **Partial** | Polling filters (log/block/pending). Missing store → error. Unknown id → `[]`. `getFilterLogs` on log filter. Lab: `evm_filters_lab.py` — **not WS `eth_subscribe`.** |
